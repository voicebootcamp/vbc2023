...
class EventbookingModelList extends RADModelList
{
	...
	protected function buildQueryJoins(JDatabaseQuery $query)
	{
		...
		$query->leftJoin('#__eb_registrants AS b ON (tbl.id = b.event_id AND b.group_id=0 AND (b.published = 1 OR (b.published = 0 AND b.payment_method LIKE "os_offline%")))')
	...
	protected function buildQueryWhere(JDatabaseQuery $query)
	{
		...
		$createdBy          = (int) $this->params->get('created_by', 0);
		...
		if ($createdBy > 0)
		{
			$query->where('tbl.created_by = ' . $createdBy);
		}
	...
	protected function applyKeywordFilter(JDatabaseQuery $query)
	{
		...
		{
			$search = $db->quote('%' . $db->escape(StringHelper::strtoupper($this->state->search), true) . '%', false);
