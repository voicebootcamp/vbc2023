<?php
namespace JExtstore\Component\JMap\Administrator\Framework\Http\Transport;
/**
...
use JExtstore\Component\JMap\Administrator\Framework\Http\Transport as ITransport;
 ...
 * @since 1.0
 */
class Socket implements ITransport {
	/**
	...
	public function __construct() {
		if (!function_exists('fsockopen') || !is_callable('fsockopen')) {
			Factory::getApplication()->enqueueMessage('The PHP function fsockopen() is not available, contact your hosting provider to enable it.', 'message');
		}
	 ...
	 *
	 * @since   11.3
	 * @throws  \JExtstore\Component\JMap\Administrator\Framework\Exception\Runtime
	 */
	public function request($method, Uri $uri, $data = null, array $headers = null, $timeout = 25, $userAgent = null) {
		...
		if (is_resource($connection)) {
			...
			if ($meta['timed_out']) {
				throw new \JExtstore\Component\JMap\Administrator\Framework\Exception\Runtime('Server connection timed out.', 'error');
			}
		} else {
			throw new \JExtstore\Component\JMap\Administrator\Framework\Exception\Runtime('Not connected to server.', 'error');
		}
	 ...
	 *
	 * @since   11.3
	 * @throws  \JExtstore\Component\JMap\Administrator\Framework\Exception\Unexpectedvalue
	 */
	protected function getResponse($content) {
		...
		$code = isset($matches[0] ) ? $matches[0] : false;
		if (is_numeric($code)) {
			$return->code = (int) $code;
		} else { // No valid response code was detected.
			throw new \JExtstore\Component\JMap\Administrator\Framework\Exception\Unexpectedvalue('No HTTP response code found.', 'notice');
		}
	...
	protected function connect(Uri $uri, $timeout = null) {
		...
		if (!$connection) {
			// Default error if missing
			if(!$err) {
				$err = 'Socket connection error';
			}
			throw new \JExtstore\Component\JMap\Administrator\Framework\Exception\Runtime($err, 'error', $errno);
		}
