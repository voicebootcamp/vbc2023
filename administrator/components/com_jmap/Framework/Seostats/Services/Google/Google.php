...
class Google extends Seostats {
	...
	public static function getSiteindexTotal($url = false) {
		...
		$totalLinksHtml = Google\Search::getSerpsIndexedLinks ( $query );
		if($totalLinksHtml && isset($totalLinksHtml[1])) {
			...
			if(!is_numeric(str_replace(array(',','.'), '', $numericValue)) && (is_numeric($explodedChunks[1][0]) || is_numeric($explodedChunks[3][0]))) {
				if(is_numeric($explodedChunks[1][0])) {
					$numericValue = preg_replace('/[^0-9]/', '', $explodedChunks[1]);
				} elseif(is_numeric($explodedChunks[3][0])) {
					$numericValue = preg_replace('/[^0-9]/', '', $explodedChunks[3]);
				}
			}
		}
	...
	public static function getRankedPageKeyword($query, $domain, $pageNumber = 0, $numResults = 100, $customHeaders = array()) {
		return Google\Search::getRankedPageKeyword ( $query, $domain, $pageNumber, $numResults, $customHeaders );
	}
}