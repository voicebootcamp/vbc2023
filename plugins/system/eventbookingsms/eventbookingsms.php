...
class plgSystemEventbookingSms extends CMSPlugin
{
	...
	private function sendSMSReminder($number)
	{
		...
		$query = $db->getQuery(true)
			...
			->where("IF($sendReminderField > 0, IF($sendReminderFrequencyField = 'd', $dayFrequencyConditionReminderBefore, $hourFrequencyConditionReminderBefore), IF($sendReminderFrequencyField = 'd', $dayFrequencyConditionReminderAfter, $hourFrequencyConditionReminderAfter))")
