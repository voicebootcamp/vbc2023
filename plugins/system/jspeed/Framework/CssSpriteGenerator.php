...
class CssSpriteGenerator {
	...
	public function CreateSprite($aFilePaths) {
		...
		foreach ( $aFilePaths as $sFile ) {
			...
			$aCdns = array_keys ( Helper::getCDNDomains ( $this->params, '', '', true ) );
			...
			if ($bFileExists && ! empty ( $sFileClass ) && in_array ( strtoupper ( $aPathParts ['extension'] ), $this->aImageTypes ) && in_array ( $iImageType, array (
					IMAGETYPE_GIF,
					IMAGETYPE_JPEG,
					IMAGETYPE_PNG
			) ) && substr ( $sFileBaseName, 0, 1 ) != '.' && $iWidth < 50 && $iHeight < 50 && $iWidth > 0 && $iHeight > 0) {
		...
		if (count ( $aFilesInfo ) > 1) {
			...
			try {
				...
				$this->bTransparent = (! empty ( $this->aFormValues ['use-transparency'] ) && in_array ( $this->aFormValues ['image-output'], array (
						'GIF',
						'PNG'
				) ));
	...
	public function __construct($ImageLibrary, $params, $bBackend = false) {
		...
		$class = 'JSpeed\ImageHandler' . ucfirst ( $ImageLibrary );
		...
		$this->aFormValues = array (
				...
				'sub' => '',
				'file-regex' => '',
				'wrap-columns' => $this->params->get ( 'combinedimage_autoformat', 'off' ),
				'build-direction' => $this->params->get ( 'combinedimage_orientation', 'vertical' ),
				'use-transparency' => 'on',
				'use-optipng' => '',
				'vertical-offset' => 50,
