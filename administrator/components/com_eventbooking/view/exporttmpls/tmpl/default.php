...
if ($saveOrder)
{
	$saveOrderingUrl = 'index.php?option=com_eventbooking&task=exporttmpl.save_order_ajax';
	...
	{
		HTMLHelper::_('sortablelist.sortable', 'exporttmplList', 'adminForm', strtolower($listDirn), $saveOrderingUrl);
	}
}
...
?>
<form action="index.php?option=com_eventbooking&view=exporttmpls" method="post" name="adminForm" id="adminForm">
    <div id="j-main-container"<?php if ($isJoomla4) echo ' class="eb-joomla4-container"'; ?>>
        <div id="filter-bar" class="btn-toolbar js-stools<?php if ($isJoomla4) echo ' js-stools-container-filters-visible'; ?>">
            <div class="filter-search btn-group pull-left">
                <label for="filter_search" class="element-invisible"><?php echo Text::_('EB_FILTER_SEARCH_EXPORTTMPLS_DESC');?></label>
                <input type="text" name="filter_search" id="filter_search" inputmode="search" placeholder="<?php echo Text::_('JSEARCH_FILTER'); ?>" value="<?php echo $this->escape($this->state->filter_search); ?>" class="hasTooltip form-control" title="<?php echo HTMLHelper::tooltipText('EB_SEARCH_EXPORTTMPLS_DESC'); ?>" />
            </div>
        ...
        <table class="adminlist table table-striped" id="exporttmplList">
            ...
            <tr>
                ...
                <th class="title">
					<?php echo HTMLHelper::_('grid.sort', Text::_('EB_NAME'), 'tbl.title', $this->state->filter_order_Dir, $this->state->filter_order); ?>
                </th>
                <th class="center">
			...
			{
				$row       = $this->items[$i];
				$link      = Route::_('index.php?option=com_eventbooking&view=exporttmpl&id=' . $row->id);
