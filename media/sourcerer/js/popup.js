    ...
    window.RegularLabs.Sourcerer.Popup = window.RegularLabs.Sourcerer.Popup || {
        ...
        setAttributes: function(string) {
            ...
            if ('css' in attributes) {
                this.setField('css_file', attributes.css);
            }
            if ('js' in attributes) {
                this.setField('js_file', attributes.js);
            }
            if ('async' in attributes) {
                this.setField('js_file_async', attributes.async.toBooleanNumber());
            }
            if ('defer' in attributes) {
                this.setField('js_file_defer', attributes.defer.toBooleanNumber());
            }
            if ('file' in attributes) {
                this.setPhpField(attributes.file, 'include');
            }
            if ('php' in attributes) {
                this.setPhpField(attributes.php, 'include');
            }
            if ('include' in attributes) {
                this.setPhpField(attributes.include, 'include');
            }
            if ('include_once' in attributes) {
                this.setPhpField(attributes.include_once, 'include_once');
            }
            if ('require' in attributes) {
                this.setPhpField(attributes.require, 'require');
            }
            if ('require_once' in attributes) {
                this.setPhpField(attributes.require_once, 'require_once');
            }
        },
