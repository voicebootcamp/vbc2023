...
class RADViewList extends RADViewHtml
{
	...
	protected function prepareView()
	{
		...
		if ($this->isAdminView)
		{
			$this->lists['filter_state']  = str_replace(['class="inputbox"', 'class="form-select"'],
				'class="input-medium form-select"', HTMLHelper::_('grid.state', $this->state->filter_state));
			$this->lists['filter_access'] = HTMLHelper::_('access.level', 'filter_access', $this->state->filter_access,
				'class="input-medium form-select" onchange="submit();"', true);
	...
	protected function addToolbar()
	{
		...
		if ($this->isAdminView)
		{
			ToolbarHelper::title(Text::_(strtoupper($languagePrefix . '_' . RADInflector::singularize($this->name) . '_MANAGEMENT')),
				'link ' . $this->name);
		}
