 ...
 * @since 2.0
 */
class WizardModel extends JMapModel implements IModelWizard {
	/**
			...
			if(!empty($manifest->placeholders)) {
				...
				foreach ($manifest->placeholders as $postfield=>$toReplace) {
					...
					if(!property_exists($manifest->postfields, $postfield)) {
						throw new JMapException(Text::_('COM_JMAP_ERROR_NOPROPERTY_EXISTS_POSTFIELDS'), 'error');
					}
					// Do evaluate
					if(!property_exists($this, $toReplace[1])) {
						throw new JMapException(Text::_('COM_JMAP_ERROR_NOPROPERTY_EXISTS_MODELOBJECT'), 'error');
					}
	...
	protected function loadManifestFile($fileName) {
		...
		if(!$fileName || !file_exists($fileName)) {
			throw new JMapException(Text::_('COM_JMAP_ERROR_NOMANIFEST_FOUND'), 'error');
		}
	...
	protected function injectRequestField($manifest) {
		...
		/**
		 * Mapping is:
		 * postfields = $this->requestArray['sqlquery_managed'][]
		 * querystringlinkparams = $this->requestArray['params'][]
		 */
		//Inject data source name first
		$dataSourceUserfriendlyName = ucfirst(str_replace('_', ' ', $this->extension));
		$this->requestArray['name'] = $dataSourceUserfriendlyName;
		$this->requestArray['params']['datasource_extension'] = $this->extension;
		
		// Missing required data to construct data source creation
		if(!property_exists($manifest, 'postfields')) {
			throw new JMapException(Text::_('COM_JMAP_ERROR_INVALID_MANIFEST_MISSING_POSTFIELDS'), 'error');
		}
		
		// Error in object syntax or not valid object syntax
		if(!is_object($manifest->postfields)) {
			throw new JMapException(Text::_('COM_JMAP_ERROR_INVALID_MANIFEST_OBJECT_POSTFIELDS'), 'error');
		}
		
		//Cycle and store postfields REQUIRED
		foreach ($manifest->postfields as $fieldName=>$fieldValue) {
			// Store in superglobal POST array
			$this->requestArray['sqlquery_managed'][$fieldName] = $fieldValue;
		}
		...
		if(property_exists($manifest, 'querystringlinkparams')) {
			...
			if(!is_object($manifest->querystringlinkparams)) {
				throw new JMapException(Text::_('COM_JMAP_ERROR_INVALID_MANIFEST_OBJECT_QSLINKPARAMS'), 'error');
			}
	 ...
	 * @return array
	 */
	public function getData($path = null): array {
		// Init extensions discovery array
		$discoveredExtensions = array();
		
		try {
			if(!$path || !is_dir($path)) {
				throw new JMapException(Text::_('COM_JMAP_EMPTY_ERROR_PATH'), 'error');
			}
			if(!class_exists('\DirectoryIterator')) {
				throw new JMapException(Text::_('COM_JMAP_SPL_LIBRARY_NOTFOUND_OLDPHPVERSION'), 'error');
			}
	...
	public function createEntityProcess() {
		...
		try {
			...
			if($sqlString != $createdDataSourceTable->sqlquery) {
				$createdDataSourceTable->ordering = null;
	...
	public function __construct($config = array(), MVCFactoryInterface $factory = null) {
		...
		if(!empty($config) && isset($config['extension'])) {
