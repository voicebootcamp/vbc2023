...
class admin_detailModelrate_adjustments_detail extends JModelLegacy
{
	...
	function _loadData()
	{
		...
		if (empty($this->_data))
		{
			...
			' WHERE id_rate_adjustments = '. $this->getDbo()->escape($this->_id_rate_adjustments);
	...
	function publish($cid = array(), $publish = 1)
	{
		...
		if (sv_count_($cid ))
		{
			...
			$query = 'UPDATE '.$this->_table_prefix.'rate_adjustments'
				...
				. ' WHERE id_rate_adjustments IN ( '.$this->getDbo()->escape($cids).' )'
  ...
  //second parameter: condition
	...
	function delete($cid = array())
	{
		...
		{
			$cids = implode( ',', $cid );
			$query = 'DELETE FROM '.$this->_table_prefix.'rate_adjustments WHERE id_rate_adjustments IN ( '.$this->getDbo()->escape($cids).' )';
			$this->_db->setQuery( $query );
			if(!$this->_db->execute()) {
				$this->setError($row->getError());
				return false;
