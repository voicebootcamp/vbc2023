...
class ImageHandlerGd {
	...
	public function resizeImage($oSprite, $oCurrentImage, $aFileInfo) {
		imagecopyresampled ( $oSprite, $oCurrentImage, $aFileInfo ['x'], $aFileInfo ['y'], 0, 0, $aFileInfo ['width'], $aFileInfo ['height'], $aFileInfo ['original-width'], $aFileInfo ['original-height'] );
	}
	public function copyImageToSprite($oSprite, $oCurrentImage, $aFileInfo, $bResize) {
		...
		if (! $bResize) {
			imagecopy ( $oSprite, $oCurrentImage, $aFileInfo ['x'], $aFileInfo ['y'], 0, 0, $aFileInfo ['width'], $aFileInfo ['height'] );
		}
	}
	...
	public function writeImage($oImage, $sExtension, $sFilename) {
		...
		switch ($sExtension) {
			...
			case 'jpeg' :
				...
				break;
			case 'gif' :
				if ($this->obj->bTransparent && ($this->obj->aFormValues ['image-num-colours'] == - 1 || $this->obj->aFormValues ['image-num-colours'] > 256 || $this->obj->aFormValues ['image-num-colours'] == 'true-colour')) {
