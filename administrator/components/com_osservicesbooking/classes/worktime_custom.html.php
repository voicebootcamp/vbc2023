...
class HTML_OSappscheduleWorktimecustom{
	 ...
	 */
	static function worktimecustom_list($option,$rows,$pageNav,$lists){
					...
					<td align="left">
						<div class="btn-group">
							<div class="input-group">
								<input type="text"  class="input-medium search-query form-control" name="keyword" value="<?php echo $lists['keyword']; ?>" />
			...
			<table class="adminlist table table-striped" width="100%">
				...
				?>
					<tr class="<?php echo "row$k";?>">
						...
						<td align="center" style="text-align:left;"><a href="<?php echo $link; ?>"><?php echo JText::_($row->worktime_date); ?></a></td>
						<td align="center" style="text-align:left;"><a href="<?php echo $link; ?>"><?php echo JText::_($row->worktime_date_to); ?></a></td>
						<td align="center" style="text-align:left;"><?php echo date($configClass['time_format'],strtotime($row->worktime_date." ".$row->start_time)); ?> </td>
						<td align="center" style="text-align:left;"><?php echo date($configClass['time_format'],strtotime($row->worktime_date." ".$row->end_time)); ?></td>
	 ...
	 * @param unknown_type $lists
	 */
	static function worktimecustom_modify($option,$row,$lists,$services)
	{
		global $mainframe, $_jversion, $jinput, $mapClass;
		$db = JFactory::getDbo();
		...
		{
			$extraClass = " joomla4customworkingtime";
		}
		?>
		<form method="POST" action="index.php" name="adminForm" id="adminForm" enctype="multipart/form-data">
			<table class="admintable <?php echo $extraClass;?>">
				<tr>
					<td class="key"><?php echo JText::_('OS_REASON'); ?>: </td>
					<td >
						<input type="text" class="<?php echo $mapClass['input-large'];?>" name="reason" value="<?php echo $row->reason?>" />
					</td>
				</tr>
				<tr>
					<td class="key"><?php echo JText::_('OS_DATE_FROM'); ?>: </td>
					<td >
						<?php echo JHtml::_('calendar',$row->worktime_date,'worktime_date','worktime_date','%Y-%m-%d','class="'.$mapClass['input-small'].' form-select required" readonly="readonly" ');?>
				...
				<tr>
					...
					<td >
						<?php echo JHtml::_('calendar',$row->worktime_date_to,'worktime_date_to','worktime_date_to','%Y-%m-%d','class="'.$mapClass['input-small'].' form-select required" readonly="readonly" ');?>
						<div id="worktime_date_invalid" style="display: none; color: red;"><?php echo JText::_('OS_THIS_FIELD_IS_REQUIRED')?></div>
					</td>
				</tr>
				<tr>
