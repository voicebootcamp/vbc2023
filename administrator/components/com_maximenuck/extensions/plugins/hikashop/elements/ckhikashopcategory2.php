...
class JFormFieldCkhikashopcategory2 extends JFormFieldList {

	protected $type = 'ckhikashopcategory2';
	...
	protected function getOptions() {
		...
		if (!JFolder::exists(JPATH_ROOT . '/administrator/components/com_hikashop')) {
			// add the root item
			$option = new stdClass();
			$option->text = JText::_('PLG_MAXIMENUCK_HIKASHOP_NOTFOUND');
			$option->value = '0';
			$options[] = $option;
		...
		require_once JPATH_ROOT . '/plugins/maximenuck/hikashop/helper/helper_hikashop.php';
		$className = 'MaximenuckHelpersourceHikashop';
		$params = new JRegistry();
		$cats = $className::getItems($params, $all = false);

		// add the root item
		$option = new stdClass();
		$option->text = JText::_('PLG_MAXIMENUCK_HIKASHOP_PRODUCT_CATEGORY');
		$option->value = '2';
		$options[] = $option;
		foreach ($cats as $cat) {
			$option = new stdClass();
