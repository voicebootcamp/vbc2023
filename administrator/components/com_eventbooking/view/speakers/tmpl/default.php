...
if ($saveOrder)
{
	$saveOrderingUrl = 'index.php?option=com_eventbooking&task=speaker.save_order_ajax';
	...
	{
		HTMLHelper::_('sortablelist.sortable', 'speakersList', 'adminForm', strtolower($listDirn), $saveOrderingUrl);
	}
}
...
?>
<form action="index.php?option=com_eventbooking&view=speakers" method="post" name="adminForm" id="adminForm">
		...
		<table class="adminlist table table-striped" id="speakersList">
			<thead>
				<tr>
					...
					<th class="title">
						<?php echo HTMLHelper::_('grid.sort', Text::_('EB_DESCRIPTION'), 'tbl.description', $this->state->filter_order_Dir, $this->state->filter_order); ?>
					</th>
					<th class="title">
						<?php echo HTMLHelper::_('grid.sort', Text::_('EB_URL'), 'tbl.url', $this->state->filter_order_Dir, $this->state->filter_order); ?>
					</th>
			...
			{
				$row       = $this->items[$i];
				$link      = Route::_('index.php?option=com_eventbooking&view=speaker&id=' . $row->id);
