...
class services_detailModelservices_detail extends JModelLegacy
{
	...
	function _loadData()
	{
		...
		if (empty($this->_data))
		{
			$query = 'SELECT * FROM '.$this->_table_prefix.'services WHERE id_services = '. $this->_id_services;
	...
	function store($data)
	{
		...
		if (!$row->id_services) {			
			...
			$row->id_services = $this->getState('services.id_services');			
		}
...
/*		if (!$row->check()) {
  ...
  //second parameter: condition
	...
	function delete($cid = array())
	{
		...
		{
			$cids = implode( ',', $cid );
			$query = 'DELETE FROM '.$this->_table_prefix.'services WHERE id_services IN ( '.$cids.' )';
			$this->_db->setQuery( $query );
			if(!$this->_db->execute()) {
				$this->setError($row->getError());
				return false;
			...
			$this->_db->setQuery("DELETE FROM #__sv_apptpro3_rate_overrides WHERE entity_type = 'service' AND entity_id IN (".$cids.")");
			if (!$this->_db->execute()) {
				$this->setError($row->getError());
				return false;
			}
