...
class plgMaximenuckHikashop extends JPlugin {

	private $type = 'hikashop';

	private $shallLoad = true;

	function __construct(&$subject, $params) {
		// does not load if the component is not installed
		$this->shallLoad = file_exists(JPATH_SITE . '/administrator/components/com_hikashop');
		if (! $this->shallLoad)
			return;
	 ...
	 */
	public function onMaximenuckRenderItemHikashop($item) {
		require_once(__DIR__ . '/helper/helper_' . $this->type . '.php');
		$hikashop = MaximenuckHelpersourceHikashop::getItems($item->params);
		$html = '<ul class="maximenuck2">';
		foreach ($hikashop as $article) {
