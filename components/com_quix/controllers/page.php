...
class QuixControllerPage extends JControllerForm
{
    ...
    protected function allowEdit($data = [], $key = 'id')
    {
        ...
        if ($user->authorise('core.edit', 'com_quix.page.' . $recordId)) {
            return true;
        }

        // Check edit own on the record asset (explicit or inherited)
        if ($user->authorise('core.edit.own', 'com_quix.page.' . $recordId)) {
    ...
    public function cancel($key = 'id')
    {
        ...
        if (!$pk) {
            ...
            $link = JRoute::_('index.php?option=com_quix&view=page&id=' . $pk);
        ...
        $redirlink = JUri::root() . 'administrator/index.php?option=com_quix&view=pages';
    ...
    public function save($key = null, $urlVar = 'id')
    {
        ...
        if ($result) {
            ...
            $menuItem = $menu->getItems('link', 'index.php?option=com_quix&view=page&id=' . $id, true);
            ...
            // saved successfully
            $url = JRoute::_('index.php?option=com_quix&view=form&layout=edit&tmpl=component&id=' . $id);
            $view = base64_encode('index.php?option=com_quix&view=page&preview=true&id=' . (int) $id);
            $edit = JRoute::_('index.php?option=com_quix&view=form&layout=iframe&builder=frontend&id=' . $id);
            echo new JResponseJson(compact('id', 'view', 'url', 'edit', 'hasMenu'));
            JFactory::getApplication()->close();
        } else {
            $err = new Exception($this->message);
    ...
    {
        $app = JFactory::getApplication();
        $app->setUserState('com_quix.edit.page.id', $model->getState('form.id'));
        ...
        if ($task == 'apply' or $task == 'save') {
            ...
            $pageJsonData = $_POST['jform']['data'];

            //$pattern = '#{"type":"image","image":"((https://|http://|//|/)?([^ ]+)?/([^ ]+).(jpg|jpeg|png))"}#';

            // preg_match_all($pattern, $pageJsonData, $matches);
            ...
            if ($model->getState('form.new')) {
                $this->setRedirect('index.php?option=com_quix&task=page.edit&id=' . $id);
            } else {
                $this->setRedirect($this->getReturnPage());
            }
        }
    ...
    public function pageCreateAjax()
    {
        ...
        $model = $this->getModel();
        if ( !$model->save(['title' => $title, 'state' => 1, 'builder' => 'frontend', 'builder_version' => QUIXNXT_VERSION, 'data' => '[]'])) {
            echo new JResponseJson(New Exception(JText::_('Failed to save')), '', 1, true);
        } else {
            $id = $model->getState('form.id');
            $link = JUri::root() . 'index.php?option=com_quix&task=page.edit&id='.$id;
