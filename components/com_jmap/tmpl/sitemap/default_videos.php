...
if($this->xslt) {
	echo "<?xml-stylesheet type='text/xsl' href='" . Uri::root() . "components/com_jmap/xslt/xml-videos-sitemap.xsl'?>" . PHP_EOL;
}
?>
...
foreach ( $this->data as $source ) {	
	...
	if ($source->type) {
		...
		if($this->sourceparams->get('xmlvideosinclude', 1)) {
			$subTemplateName = $this->_layout . '_videos_' . $source->type . '.php';
			if (file_exists ( JPATH_COMPONENT_SITE . '/tmpl/sitemap/' . $subTemplateName )) {
				echo $this->loadTemplate ( 'videos_' . $source->type );
			}
		}
	}
}
