...
class stripe_transactionsController extends JControllerForm
{
	...
	function __construct( $default = array())
	{
		...
		$this->registerTask( 'export_stripe', 'export_data' );
	...
	function display($cachable=false, $urlparams=false) {
		...
		rsappt_pro3Helper::addSubmenu('stripe_transactions');
	...
	function export_data(){
		...
		$sql = "SELECT ".
			  " #__sv_apptpro3_stripe_transactions.id_stripe_transactions, #__sv_apptpro3_stripe_transactions.stripe_txn_id, ".
			  " #__sv_apptpro3_stripe_transactions.amount, #__sv_apptpro3_stripe_transactions.seller_message, #__sv_apptpro3_stripe_transactions.descrition,".
			  " #__sv_apptpro3_requests.id_requests AS RequestID,".
			  ...
			  " FROM ".
			  " #__sv_apptpro3_stripe_transactions INNER JOIN ".
			  " #__sv_apptpro3_requests ON #__sv_apptpro3_stripe_transactions.request_id = ".
			  ...
			  " #__sv_apptpro3_resources.id_resources ".
				" WHERE #__sv_apptpro3_stripe_transactions.id_stripe_transactions IN (".implode(",", $uid).")";
		//echo $sql;
		//exit;
		
		ob_end_clean();
			
		$file_name = 'export_sv_stripe_transactions.csv';
		...
		try{
			...
			logIt($e->getMessage(), "be_ctrl_stripe_trans", "", "");
