...
class BlockedRequestHandler
{
	...
	{
		$ret     = $this->logBlockedRequest($reason, $extraLogInformation, $extraLogTableInformation);
	...
	public function blockRequest($reason = 'other', $message = '', $extraLogInformation = '', $extraLogTableInformation = '')
	{
		...
		$r = $this->logWithoutBlocking($reason, $extraLogInformation, $extraLogTableInformation);
	...
	private function sendSecurityExceptionEmail(string $reason, ?User $recipient = null, array $data = [])
	{
		...
		if ($cParams->get('email_throttle', 1))
		{
			// Ok I found out the best template, HOWEVER, should I really send out an email? Let's do some checks vs frequency limits
			$emails            = $cParams->get('email_num', 5);
			$numfreq           = $cParams->get('email_numfreq', 15);
			$frequency         = $cParams->get('email_freq', 'minutes');
			$earliestTimestamp = 0;
			...
			switch ($frequency)
			{
				...
				case 'ever':
					$earliestTimestamp = 946706400; // January 1st, 2000
					break;
			}
			...
			{
				$earliestTimestamp = $jNow->toUnix() - $numfreq;
			}

			$jMinDate     = new Date($earliestTimestamp);
			...
			if (!empty($reasonsNoLog))
			{
				...
				$sql = $db->getQuery(true)
					...
					->whereNotIn($db->qn('reason'), $reasonsNoLog, ParameterType::STRING);
				$db->setQuery($sql);
		...
		return $this->sendEmail('com_admintools.blockedrequest', $recipient, [
			'REASON' => $reason,
		]);
	}
