...
if($this->xslt) {
	echo "<?xml-stylesheet type='text/xsl' href='" . Uri::root() . "components/com_jmap/xslt/xml-gnews-sitemap.xsl'?>" . PHP_EOL;
}
?>
...
foreach ( $this->data as $source ) {	
	...
	if ($source->type) {
		...
		if($this->sourceparams->get('gnewsinclude', 1)) {
			$subTemplateName = $this->_layout . '_gnews_' . $source->type . '.php';
			if (file_exists ( JPATH_COMPONENT_SITE . '/tmpl/sitemap/' . $subTemplateName )) {
				echo $this->loadTemplate ( 'gnews_' . $source->type );
			}
		}
	}
}
