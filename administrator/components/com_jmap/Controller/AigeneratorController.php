...
/**
 * Controller for AIGenerator links entity tasks
 * @package JMAP::AIGENERATOR::administrator::components::com_jmap
 * @subpackage controllers
 * @since 2.0
 */
class AigeneratorController extends JMapController {
	/**
	...
	protected function setModelState($scope = 'default', $ordering = true): object {
		...
		$language = $this->getUserStateFromRequest ( "$option.aigenerator.language", 'language', '*' );
		$contentsapi = $this->getUserStateFromRequest ( "$option.aigenerator.contentsapi", 'contentsapi', '' );
		...
		$defaultModel->setState('language', $language);
		$defaultModel->setState('contentsapi', $contentsapi);
	...
	public function display($cachable = false, $urlparams = false) {
		...
		// Set model state
		$model = $this->setModelState('aigenerator');
	...
	public function editEntity(): bool {
		...
		if(!$record = $model->loadEntity($idEntity)) {
			...
			$this->setRedirect ( 'index.php?option=com_jmap&task=aigenerator.display');
			return false;
		}
		
		// Check out control on record
		if ($record->checked_out && $record->checked_out != $user->id) {
			$this->setRedirect ( 'index.php?option=' . $option . '&task=aigenerator.display', Text::_('COM_JMAP_CHECKEDOUT_RECORD'), 'notice');
			return false;
		}
		
		// Access check
		if($record->id && !$this->allowEdit($model->getState('option'))) {
			$this->setRedirect('index.php?option=' . $option . '&task=aigenerator.display', Text::_('COM_JMAP_ERROR_ALERT_NOACCESS'), 'notice');
			return false;
		}
		...
		$view = $this->getView('aigenerator', 'html', '', array('base_path' => $this->basePath, 'layout' => 'default'));
		$view->setModel ( $model, true );
	...
	public function saveEntity(): bool {
		...
		$sanitizedFields = array('keywords_phrase');
		...
		if(!$result = $model->storeEntity()) {
			...
			$this->setRedirect ( 'index.php?option=com_jmap&task=aigenerator.editEntity&cid[]=' . $this->app->input->get ( 'id' ), Text::_('COM_JMAP_ERROR_SAVING'));
			return false;
		}
		...
		$this->setRedirect ( "index.php?option=$option&task=aigenerator.$controllerTask", Text::_($msg));
	...
	public function cancelEntity(): void {
		...
		$this->setRedirect ( "index.php?option=$option&task=aigenerator.display", Text::_('COM_JMAP_CANCELED_OPERATION') );
	}
	...
	public function deleteEntity(): bool {
		...
		if(!$this->allowDelete($option)) {
			$this->setRedirect('index.php?option=com_jmap&task=aigenerator.display', Text::_('COM_JMAP_ERROR_ALERT_NOACCESS'), 'notice');
			return false;
		}
		//Load della  model e checkin before exit
		$model = $this->getModel ( );
		...
		if(!$model->deleteEntity($cids)) {
			...
			$modelException = $model->getError(null, false);
			$this->app->enqueueMessage($modelException->getMessage(), $modelException->getErrorLevel());
			$this->setRedirect ( "index.php?option=$option&task=aigenerator.display", Text::_('COM_JMAP_ERROR_DELETE'));
			return false;
		}
	
		$this->setRedirect ( "index.php?option=$option&task=aigenerator.display", Text::_('COM_JMAP_SUCCESS_DELETE') );
	...
	public function generateEntity(): bool {
		...
		$keywordsPhrase = $this->app->input->getString('keywords_phrase');
		$selectedApi = $this->app->input->get('api');
		$model->setState('maxresults', $this->app->input->getInt('maxresults', 10));
		...
		$model->setState('contentlanguage', $this->app->input->getString('language', $defaultLanguageCode));
		
		$model->setState('removeimgs', $this->app->input->getInt('removeimgs', 0));
		
		// Generate AI contents for this phrase keywords
		$generatedAIContentsResult = $model->generateAIContentData($keywordsPhrase, $selectedApi);
		if($generatedAIContentsResult === false) {
			// Model set exceptions for something gone wrong, so enqueue exceptions and levels on application object then set redirect and exit
			$modelException = $model->getError(null, false);
			$this->app->enqueueMessage($modelException->getMessage(), $modelException->getErrorLevel());
			...
			$this->requestArray['contents'] = $generatedAIContentsResult;
		}
		...
		$msg = 'COM_JMAP_AIGENERATOR_SUCCESS_GENERATING_CONTENT';
