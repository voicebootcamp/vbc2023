...
class Document
{
     ...
     * @param string $name
     */
    public static function removeScriptTag(&$string, $name)
    {
        $name = RegEx::quote($name);
        $name = str_replace('\*', '[^"]*', $name);

        $string = RegEx::replace('\s*<script [^>]*data-asset-name="' . $name . '"[^>]*></script>', '', $string);
    }
     ...
     * @param string $name
     */
    public static function removeStyleTag(&$string, $name)
    {
        $name = RegEx::quote($name);
        $name = str_replace('\*', '[^"]*', $name);

        $string = RegEx::replace('\s*<link [^>]*data-asset-name="' . $name . '"[^>]*( /)?>', '', $string);
    }
