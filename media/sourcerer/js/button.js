...
(function() {
    ...
    window.RegularLabs.Sourcerer.Button = window.RegularLabs.Sourcerer.Button || {
        ...
        insertText: function(editor_name) {
            ...
            let css_code = this.form.editors['css_code'].getValue().trim();
            if (css_code.length) {
                if (css_code.hasLineBreaks()) {
                    css_code = css_code.indent();
                }

                pre_php.push('$document->getWebAssetManager()->addInlineStyle(\'' + css_code.escapeQuotes() + '\');');
            }

            let js_code = this.form.editors['js_code'].getValue().trim();
            if (js_code.length) {
                if (js_code.hasLineBreaks()) {
                    js_code = js_code.indent();
                }

                const js_code_position = this.form['js_code_position'].value;
                if (this.form['js_code_position'].value === 'inline') {
                    codes.push('<script>' + js_code + '</script>');
                } else {
                    pre_php.push("$document->getWebAssetManager()->addInlineScript("
                        + "\n\t'" + js_code.escapeQuotes() + "',"
                        + "\n\t['position' => '" + js_code_position + "']"
                        + "\n);");
                }
            }

            let php_code = this.form.editors['php_code'].getValue().trim();
            if (php_code.length) {
                // Add a semicolon to the end of the code
                php_code = php_code + ';';
                php_code = php_code.replace(/;;$/, ';');

                pre_php.push(php_code);
            }
            ...
            string = this.joinPrePhp(string, pre_php);
            ...
            const php_file = this.form['php_file'].value.trim();
            if (php_file.length) {
                attributes.push(this.form['php_include_method'].value.trim() + '="' + php_file + '"');
            }

            const css_file = this.form['css_file'].value.trim();
            if (css_file.length) {
                attributes.push('css="' + css_file + '"');
            }

            const js_file = this.form['js_file'].value.trim();
            if (js_file.length) {
                attributes.push('js="' + js_file + '"');

                if (this.form['js_file_async'].value === '1') {
                    attributes.push('async="true"');
                }

                if (this.form['js_file_defer'].value === '1') {
        ...
        joinPrePhp: function(string, pre_php) {
            ...
            [pre_php, string] = this.preparePrePhp(string, pre_php);

            return (pre_php + '\n' + string).trim();
        },

        preparePrePhp: function(string, pre_php) {
            const sting_starts_with_php = string.substr(0, 5) === '<?php';

            pre_php = pre_php.join('\n');

            if (sting_starts_with_php) {
                string = string.substr(5).trim();
