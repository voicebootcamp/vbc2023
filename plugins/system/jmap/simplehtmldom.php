...
define('JMAP_HDOM_TYPE_ELEMENT', 1);
define('JMAP_HDOM_TYPE_COMMENT', 2);
define('JMAP_HDOM_TYPE_TEXT', 3);
define('JMAP_HDOM_TYPE_ENDTAG', 4);
define('JMAP_HDOM_TYPE_ROOT', 5);
define('JMAP_HDOM_TYPE_UNKNOWN', 6);
define('JMAP_HDOM_QUOTE_DOUBLE', 0);
define('JMAP_HDOM_QUOTE_SINGLE', 1);
define('JMAP_HDOM_QUOTE_NO', 3);
define('JMAP_HDOM_INFO_BEGIN', 0);
define('JMAP_HDOM_INFO_END', 1);
define('JMAP_HDOM_INFO_QUOTE', 2);
define('JMAP_HDOM_INFO_SPACE', 3);
define('JMAP_HDOM_INFO_TEXT', 4);
define('JMAP_HDOM_INFO_INNER', 5);
define('JMAP_HDOM_INFO_OUTER', 6);
define('JMAP_HDOM_INFO_ENDSPACE', 7);

/** The default target charset */
defined('JMAP_DEFAULT_TARGET_CHARSET') || define('JMAP_DEFAULT_TARGET_CHARSET', 'UTF-8');

/** The default <br> text used instead of <br> tags when returning text */
defined('JMAP_DEFAULT_BR_TEXT') || define('JMAP_DEFAULT_BR_TEXT', "\r\n");

/** The default <span> text used instead of <span> tags when returning text */
defined('JMAP_DEFAULT_SPAN_TEXT') || define('JMAP_DEFAULT_SPAN_TEXT', ' ');

/** The maximum file size the parser should load */
defined('JMAP_MAX_FILE_SIZE') || define('JMAP_MAX_FILE_SIZE', 600000);

/** Contents between curly braces "{" and "}" are interpreted as text */
define('JMAP_HDOM_SMARTY_AS_TEXT', 1);

// helper functions
// -----------------------------------------------------------------------------
// get html dom from file
// $maxlen is defined in the code as PHP_STREAM_COPY_ALL which is defined as -1.
function jmap_simplehtmldom_file_get_html(
	$url,
	...
	$target_charset = JMAP_DEFAULT_TARGET_CHARSET,
	$stripRN = true,
	$defaultBRText = JMAP_DEFAULT_BR_TEXT,
	$defaultSpanText = JMAP_DEFAULT_SPAN_TEXT)
{
	// Ensure maximum length is greater than zero
	if($maxLen <= 0) { $maxLen = JMAP_MAX_FILE_SIZE; }

	// We DO force the tags to be terminated.
	$dom = new JMapSimpleHtmlDom(
		null,
		$lowercase,
		$forceTagsClosed,
		$target_charset,
...
function jmap_simplehtmldom_str_get_html(
	$str,
	$lowercase = true,
	$forceTagsClosed = true,
	$target_charset = JMAP_DEFAULT_TARGET_CHARSET,
	...
	$defaultSpanText = JMAP_DEFAULT_SPAN_TEXT)
{
	...
	if (empty($str) || strlen($str) > JMAP_MAX_FILE_SIZE) {
		$dom->clear();
		return false;
	}
 ...
 */
class JMapSimpleHtmlDomNode
{
	/**
	 * Node type
	 *
	 * Default is {@see JMAP_HDOM_TYPE_TEXT}
	 *
	 * @var int
	 */
	public $nodetype = JMAP_HDOM_TYPE_TEXT;
	...
	// The "info" array - see JMAP_HDOM_INFO_... for what each element contains.
	public $_ = array();
	 ...
	 *
	 * Adds itself to the list of DOM Nodes {@see JMapSimpleHtmlDom::$nodes}
	 */
	...
	function dump_node($echo = true)
	{
		...
		$string .= " JMAP_HDOM_INNER_INFO: '";

		if (isset($node->_[JMAP_HDOM_INFO_INNER])) {
			$string .= $node->_[JMAP_HDOM_INFO_INNER] . "'";
		} else {
			$string .= ' NULL ';
		}
	...
	function innertext()
	{
		if (isset($this->_[JMAP_HDOM_INFO_INNER])) {
			return $this->_[JMAP_HDOM_INFO_INNER];
		}

		if (isset($this->_[JMAP_HDOM_INFO_TEXT])) {
			return $this->dom->restore_noise($this->_[JMAP_HDOM_INFO_TEXT]);
		}
	...
	function outertext()
	{
		...
		if (isset($this->_[JMAP_HDOM_INFO_OUTER])) {
			return $this->_[JMAP_HDOM_INFO_OUTER];
		}
		...
		// render begin tag
		if ($this->dom && $this->dom->nodes[$this->_[JMAP_HDOM_INFO_BEGIN]]) {
			$ret = $this->dom->nodes[$this->_[JMAP_HDOM_INFO_BEGIN]]->makeup();
		} else {
			$ret = '';
		}
		...
		// render inner text
		if (isset($this->_[JMAP_HDOM_INFO_INNER])) {
			// If it's a br tag...  don't return the JMAP_HDOM_INNER_INFO that we
			// may or may not have added.
			if ($this->tag !== 'br') {
				$ret .= $this->_[JMAP_HDOM_INFO_INNER];
			}
		} else {
			if ($this->nodes) {
				foreach ($this->nodes as $n) {
		...
		// render end tag
		if (isset($this->_[JMAP_HDOM_INFO_END]) && $this->_[JMAP_HDOM_INFO_END] != 0) {
			$ret .= '</' . $this->tag . '>';
		}
	...
	function text()
	{
		...
		switch ($this->nodetype) {
			case JMAP_HDOM_TYPE_TEXT: return $this->dom->restore_noise($this->_[JMAP_HDOM_INFO_TEXT]);
			case JMAP_HDOM_TYPE_COMMENT: return '';
			case JMAP_HDOM_TYPE_UNKNOWN: return '';
		}
		...
		// In rare cases, (always node type 1 or JMAP_HDOM_TYPE_ELEMENT - observed
	...
	function makeup()
	{
		...
		foreach ($this->attr as $key => $val) {
			...
			$ret .= $this->_[JMAP_HDOM_INFO_SPACE][$i][0];

			//no value attr: nowrap, checked selected...
			if ($val === true) {
				$ret .= $key;
			} else {
				switch ($this->_[JMAP_HDOM_INFO_QUOTE][$i])
				{
					case JMAP_HDOM_QUOTE_DOUBLE: $quote = '"'; break;
					case JMAP_HDOM_QUOTE_SINGLE: $quote = '\''; break;
					default: $quote = '';
				}

				$ret .= $key
				. $this->_[JMAP_HDOM_INFO_SPACE][$i][1]
				. '='
				. $this->_[JMAP_HDOM_INFO_SPACE][$i][2]
				. $quote
				. $val
				. $quote;
			}
		...
		return $ret . $this->_[JMAP_HDOM_INFO_ENDSPACE] . '>';
	}
	...
	function find($selector, $idx = null, $lowercase = false)
	{
		...
		for ($c = 0; $c < $count; ++$c) {
			...
			if (!isset($this->_[JMAP_HDOM_INFO_BEGIN])) { return array(); }

			$head = array($this->_[JMAP_HDOM_INFO_BEGIN] => 1);
			$cmd = ' '; // Combinator
	 ...
	 * {@see JMapSimpleHtmlDomNode::parse_selector()}
	 *
	...
	protected function seek($selector, &$ret, $parent_cmd, $lowercase = false)
	{
		...
		if ($parent_cmd === ' ') { // Descendant Combinator
			...
			$end = (!empty($this->_[JMAP_HDOM_INFO_END])) ? $this->_[JMAP_HDOM_INFO_END] : 0;
			if ($end == 0) {
				$parent = $this->parent;
				while (!isset($parent->_[JMAP_HDOM_INFO_END]) && $parent !== null) {
					$end -= 1;
					$parent = $parent->parent;
				}
				$end += $parent->_[JMAP_HDOM_INFO_END];
			}

			// Get list of target nodes
			$nodes_start = $this->_[JMAP_HDOM_INFO_BEGIN] + 1;
		...
		foreach($nodes as $node) {
			...
			if ($pass && $id !== '' && isset($node->attr['id'])) {
				...
				$nodeTemp = explode(' ', trim($node->attr['id']));
				$node_id = $nodeTemp[0];
			...
			if ($pass) $ret[$node->_[JMAP_HDOM_INFO_BEGIN]] = 1;
			unset($node);
		}
	...
	function __set($name, $value)
	{
		...
		switch ($name) {
			case 'outertext': return $this->_[JMAP_HDOM_INFO_OUTER] = $value;
			case 'innertext':
				if (isset($this->_[JMAP_HDOM_INFO_TEXT])) {
					return $this->_[JMAP_HDOM_INFO_TEXT] = $value;
				}
				return $this->_[JMAP_HDOM_INFO_INNER] = $value;
		}

		if (!isset($this->attr[$name])) {
			$this->_[JMAP_HDOM_INFO_SPACE][] = array(' ', '', '');
			$this->_[JMAP_HDOM_INFO_QUOTE][] = JMAP_HDOM_QUOTE_DOUBLE;
		}
...
class JMapSimpleHtmlDom
{
	 ...
	 * string length of ({@see JMapSimpleHtmlDom::$doc}).
	 *
	 ...
	 * Current character
	 *
	 * Holds the current character at position {@see JMapSimpleHtmlDom::$pos} in
	 * the document {@see JMapSimpleHtmlDom::$doc}
	 *
		...
		$options = 0)
	{
		...
		if($options & JMAP_HDOM_SMARTY_AS_TEXT) { // Strip Smarty scripts
		...
		$this->parse();
		// end
		$this->root->_[JMAP_HDOM_INFO_END] = $this->cursor;
		$this->parse_charset();
		...
		$defaultSpanText = JMAP_DEFAULT_SPAN_TEXT)
	{
		...
		$this->root = new JMapSimpleHtmlDomNode($this);
		$this->root->tag = 'root';
		$this->root->_[JMAP_HDOM_INFO_BEGIN] = -1;
		$this->root->nodetype = JMAP_HDOM_TYPE_ROOT;
	...
	protected function parse()
	{
		while (true) {
			...
			$node = new JMapSimpleHtmlDomNode($this);
			++$this->cursor;
			$node->_[JMAP_HDOM_INFO_TEXT] = $s;
	...
	protected function read_tag()
	{
		...
		if ($this->char === '/') {
			...
			if ($parent_lower !== $tag_lower) {
				...
				if (isset($this->optional_closing_tags[$parent_lower])
					...
					$this->parent->_[JMAP_HDOM_INFO_END] = 0;
					...
					if (strtolower($this->parent->tag) !== $tag_lower) {
						...
						$this->parent->_[JMAP_HDOM_INFO_END] = $this->cursor;
					...
					$this->parent->_[JMAP_HDOM_INFO_END] = 0; // No end tag
					$org_parent = $this->parent;
		...
		// start tag
		$node = new JMapSimpleHtmlDomNode($this);
		$node->_[JMAP_HDOM_INFO_BEGIN] = $this->cursor;
		++$this->cursor;
		...
		if (isset($tag[0]) && $tag[0] === '!') {
			$node->_[JMAP_HDOM_INFO_TEXT] = '<' . $tag . $this->copy_until_char('>');

			if (isset($tag[2]) && $tag[1] === '-' && $tag[2] === '-') { // Comment ("<!--")
				$node->nodetype = JMAP_HDOM_TYPE_COMMENT;
				$node->tag = 'comment';
			} else { // Could be doctype or CDATA but we don't care
				$node->nodetype = JMAP_HDOM_TYPE_UNKNOWN;
				$node->tag = 'unknown';
			}

			if ($this->char === '>') { $node->_[JMAP_HDOM_INFO_TEXT] .= '>'; }
		...
		if ($pos = strpos($tag, '<') !== false) {
			$tag = '<' . substr($tag, 0, -1);
			$node->_[JMAP_HDOM_INFO_TEXT] = $tag;
			$this->link_nodes($node, false);
		...
		if (!preg_match('/^\w[\w:-]*$/', $tag)) {
			$node->_[JMAP_HDOM_INFO_TEXT] = '<' . $tag . $this->copy_until('<>');
		...
		$node->nodetype = JMAP_HDOM_TYPE_ELEMENT;
		$tag_lower = strtolower($tag);
		...
		do {
			...
			if ($this->pos >= $this->size - 1 && $this->char !== '>') {
				$node->nodetype = JMAP_HDOM_TYPE_TEXT;
				$node->_[JMAP_HDOM_INFO_END] = 0;
				$node->_[JMAP_HDOM_INFO_TEXT] = '<' . $tag . $space[0] . $name;
				$node->tag = 'text';
				$this->link_nodes($node, false);
				return true;
			}
			...
			if ($this->doc[$this->pos - 1] == '<') {
				...
				$node->attr = array();
				$node->_[JMAP_HDOM_INFO_END] = 0;
				$node->_[JMAP_HDOM_INFO_TEXT] = substr(
					$this->doc,
					$begin_tag_pos,
			...
			if ($name !== '/' && $name !== '') { // this is a attribute name
				...
				if ($this->char === '=') { // attribute with value
					...
					//no value attr: nowrap, checked selected...
					$node->_[JMAP_HDOM_INFO_QUOTE][] = JMAP_HDOM_QUOTE_NO;
					$node->attr[$name] = true;
					if ($this->char != '>') { $this->char = $this->doc[--$this->pos]; } // prev
				}

				$node->_[JMAP_HDOM_INFO_SPACE][] = $space;
		...
		$this->link_nodes($node, true);
		$node->_[JMAP_HDOM_INFO_ENDSPACE] = $space[0];

		// handle empty tags (i.e. "<div/>")
		if ($this->copy_until_char('>') === '/') {
			$node->_[JMAP_HDOM_INFO_ENDSPACE] .= '/';
			$node->_[JMAP_HDOM_INFO_END] = 0;
		} else {
			// reset parent
		...
		if ($node->tag === 'br') {
			$node->_[JMAP_HDOM_INFO_INNER] = $this->default_br_text;
		}
	...
	protected function parse_attr($node, $name, &$space)
	{
		...
		switch ($this->char) {
			case '"': // value is anything between double quotes
				$node->_[JMAP_HDOM_INFO_QUOTE][] = JMAP_HDOM_QUOTE_DOUBLE;
				$this->char = (++$this->pos < $this->size) ? $this->doc[$this->pos] : null; // next
				$node->attr[$name] = $this->restore_noise($this->copy_until_char('"'));
				$this->char = (++$this->pos < $this->size) ? $this->doc[$this->pos] : null; // next
				break;
			case '\'': // value is anything between single quotes
				$node->_[JMAP_HDOM_INFO_QUOTE][] = JMAP_HDOM_QUOTE_SINGLE;
				$this->char = (++$this->pos < $this->size) ? $this->doc[$this->pos] : null; // next
				$node->attr[$name] = $this->restore_noise($this->copy_until_char('\''));
				$this->char = (++$this->pos < $this->size) ? $this->doc[$this->pos] : null; // next
				break;
	 ...
	 *
	 * Noise is stored to {@see JMapSimpleHtmlDom::$noise}
	 *
	 ...
	 *
	 * Noise is restored from {@see JMapSimpleHtmlDom::$noise}
	 *
	...
	{
		return @jmap_simplehtmldom_str_get_html("<$name>$value</$name>")->firstChild();
	}

	function createTextNode($value)
	{
		return @end(jmap_simplehtmldom_str_get_html($value)->nodes);
	}
