 ...
 */
(function(){'use strict';window.RegularLabs=window.RegularLabs||{};window.RegularLabs.Sourcerer=window.RegularLabs.Sourcerer||{};window.RegularLabs.Sourcerer.Popup=window.RegularLabs.Sourcerer.Popup||{form:null,options:{},init:function(editor_name){if(!parent.RegularLabs.Sourcerer.Button){document.querySelector('body').innerHTML='<div class="alert alert-error">This page cannot function on its own.</div>';return;}
const code_editor=Joomla.editors.instances['code'];try{code_editor.getValue();}catch(err){setTimeout(()=>{RegularLabs.Sourcerer.Popup.init(editor_name);},100);return;}
this.options=parent.Joomla.getOptions?parent.Joomla.getOptions('rl_sourcerer',{}):parent.Joomla.optionsStorage.rl_sourcerer||{};const form=document.getElementById('sourcererForm');form.editors=Joomla.editors.instances;parent.RegularLabs.Sourcerer.Button.form=form;const source_editor=parent.Joomla.editors.instances[editor_name];if(!source_editor){return;}
...
if('trim'in attributes){this.setField('trim',attributes.trim.toBooleanNumber());}if('css'in attributes){this.setField('css_file',attributes.css);}
if('js'in attributes){this.setField('js_file',attributes.js);}
if('async'in attributes){this.setField('js_file_async',attributes.async.toBooleanNumber());}
if('defer'in attributes){this.setField('js_file_defer',attributes.defer.toBooleanNumber());}
if('file'in attributes){this.setPhpField(attributes.file,'include');}
if('php'in attributes){this.setPhpField(attributes.php,'include');}
if('include'in attributes){this.setPhpField(attributes.include,'include');}
if('include_once'in attributes){this.setPhpField(attributes.include_once,'include_once');}
if('require'in attributes){this.setPhpField(attributes.require,'require');}
if('require_once'in attributes){this.setPhpField(attributes.require_once,'require_once');}},setPhpField:function(value,method){this.setField('php_file',value);this.setField('php_include_method',method);},getAttributes:function(string){const attributes={};let regex=new RegExp('^0 ?');if(string.match(regex)){attributes.raw=true;string=string.replace(/^0/,'').trim();}
