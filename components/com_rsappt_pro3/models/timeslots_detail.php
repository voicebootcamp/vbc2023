...
class admin_detailModeltimeslots_detail extends JModelLegacy
{
	...
	{
		$query = "SELECT #__users.name FROM #__users JOIN #__sv_apptpro3_timeslots ON #__sv_apptpro3_timeslots.checked_out = #__users.id ".
		" WHERE #__sv_apptpro3_timeslots.id_timeslots = ". $this->getDbo()->escape($this->_id_timeslots);			
	...
	function _loadData()
	{
		...
		if (empty($this->_data))
		{
			$query = 'SELECT * FROM '.$this->_table_prefix.'timeslots WHERE id_timeslots = '. $this->getDbo()->escape($this->_id_timeslots);
			$this->_db->setQuery($query);
	...
	function store($data)
	{
		//DEVNOTE: Load table class from com_rsappt_pro3/tables/timeslots_detail.php	
		$row = $this->getTable();
		...
		if (!$row->id_timeslots) {
			$where = 'id_timeslots = ' . $this->getDbo()->escape($row->id_timeslots) ;
	...
	function publish($cid = array(), $publish = 1)
	{
		...
		if (sv_count_($cid ))
		{
			...
			$query = 'UPDATE '.$this->_table_prefix.'timeslots'
				...
				. ' WHERE id_timeslots IN ( '.$this->getDbo()->escape($cids).' )'
  ...
  //second parameter: condition
	...
	function delete($cid = array())
	{
		...
		{
			$cids = implode( ',', $cid );
			$query = 'DELETE FROM '.$this->_table_prefix.'timeslots WHERE id_timeslots IN ( '.$this->getDbo()->escape($cids).' )';
			$this->_db->setQuery( $query );
			if(!$this->_db->execute()) {
				$this->setError($row->getError());
				return false;
