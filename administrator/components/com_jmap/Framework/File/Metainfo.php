 ...
 * @since 3.5
 */
class Metainfo extends BaseError {
	/**
	 * Database connector
	 * 
	 * @access private
	...
	public function import() {
		...
		try {
			...
			if($tmpFileExtension != 'csv') {
				throw new JMapException(Text::_('COM_JMAP_METAINFO_EXT_ERROR'), 'error');
			}
			...
			if(!count($dbQueryArray)) {
				throw new JMapException(Text::_('COM_JMAP_METAINFO_NO_IMPORT_DATA_FOUND'), 'warning');
			}
			...
			$jmapException = new JMapException(Text::sprintf('COM_JMAP_METAINFO_ERROR_STORING_DATA', $e->getMessage()), 'error');
