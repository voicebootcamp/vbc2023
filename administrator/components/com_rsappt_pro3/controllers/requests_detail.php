...
class requests_detailController extends JControllerForm
{
	...
	*/
	function edit($key=null, $urlVar=null)
	{
		...
		$model = $this->getModel('requests_detail');
		$model->checkout();
	}
	...
	*/
	function save($key=null, $urlVar=null)
	{
		...
		$model->checkin();
		$this->setRedirect( 'index.php?option=com_rsappt_pro3&controller=requests',$msg );
	}
	...
	*/
	function cancel($key=null)
	{
		...
		$model = $this->getModel('requests_detail');
		$model->checkin();
		if($jinput->getString("frompage") == "PayPal"){
			$this->setRedirect( 'index.php?option=com_rsappt_pro3&controller=paypal_transactions');
		} else if($jinput->getString("frompage") == "AuthNet"){
			$this->setRedirect( 'index.php?option=com_rsappt_pro3&controller=authnet_transactions');
		} else if($jinput->getString("frompage") == "AuthNetAIM"){
			$this->setRedirect( 'index.php?option=com_rsappt_pro3&controller=authnet_aim_transactions');
		} else if($jinput->getString("frompage") == "2CO"){
			$this->setRedirect( 'index.php?option=com_rsappt_pro3&controller=_2co_transactions');
		} else if($jinput->getString("frompage") == "UC"){
			$this->setRedirect( 'index.php?option=com_rsappt_pro3&controller=user_credit_detail&task=edit&cid[]='.$jinput->getString("frompage_item")."&credit_type=".$jinput->getString("credit_type"));
//			$this->setRedirect( 'index.php?option=com_rsappt_pro3&controller=user_credit_detail&task=edit&cid[]='.$jinput->getString("frompage_item")."&type=".$jinput->getString("credit_type"));
		} else if($jinput->getString("frompage") == "GoogleWallet"){
			$this->setRedirect( 'index.php?option=com_rsappt_pro3&controller=google_wallet_transactions');
		} else if($jinput->getString("frompage") == "Stripe"){
	...
	*/
	...
	/** function saveorder
	*
	* saveorder of the requests items
	* 			
	* @param array cid		- array of id
