...
class EventbookingModelCalendar extends EventbookingModelCommoncalendar
{
	...
	public function getEventsByWeek()
	{
		...
		if ($locationIds)
		{
			$query->where('a.location_id IN (' . implode(',', $locationIds) . ')');
		}
	...
	protected function replaceEventCustomFields($rows)
	{
		...
		foreach ($rows as $row)
		{
			...
			{
				if (is_object($fieldsValue->get($field, '')))
				{
					continue;
				}

				$row->short_description = str_replace('[' . strtoupper($field) . ']', $fieldsValue->get($field, ''), $row->short_description);
			}
		}
	}
}
