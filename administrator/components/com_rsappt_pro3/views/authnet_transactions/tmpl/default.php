...
function myonsubmit(){
   ...
   if (task)    
	if ((task=='add')||(task=='edit')||(task=='publish')||(task=='unpublish')||(task=='remove') )
	 {
	  form.controller.value="authnet_transactions_detail";
	 }
	return true;	
	//
}
...
<table class="table table-striped" >
  ...
  <?php
	...
	$link 	= JRoute::_( 'index.php?option=com_rsappt_pro3&controller=authnet_transactions_detail&task=edit&cid[]='. $row->id_authnet_transactions );
	$link_to_request = JRoute::_( 'index.php?option=com_rsappt_pro3&controller=requests_detail&task=edit&cid[]='. $row->x_invoice_num."&frompage=AuthNet" );
	$checked 	= JHTML::_('grid.checkedout', $row, $i, 'id_authnet_transactions');
  ?>
  <tr class="<?php echo "row$k"; ?>">
    <td align="center"><input type="checkbox" id="cb<?php echo $i;?>" name="cid[]" value="<?php echo $row->id_authnet_transactions; ?>" onclick="Joomla.isChecked(this.checked);" /></td>
...
<table class="adminheading">
  ...
  <input type="hidden" name="controller" value="authnet_transactions" />
