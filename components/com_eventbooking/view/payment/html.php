...
class EventbookingViewPaymentHtml extends RADViewHtml
{
	...
	protected function setBaseFormData($rowRegistrant, $event)
	{
		...
		$lists['exp_month'] = HTMLHelper::_('select.integerlist', 1, 12, 1, 'exp_month', ['list.select'   => $expMonth,
		                                                                                  'option.format' => '%02d',
		                                                                                  'list.attr'     => 'class="input-medium form-select"',
		                                                                                  'id'            => 'exp_month', ]);
		$lists['exp_year']  = HTMLHelper::_('select.integerlist', $currentYear, $currentYear + 10, 1, 'exp_year', ['list.select' => $expYear,
		                                                                                                           'list.attr'   => 'class="input-medium form-select"',
		                                                                                                           'id'          => 'exp_year', ]);
