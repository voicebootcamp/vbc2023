...
class Menu {
	...
	public static function getMenuItems($menuTitle = null, $withPriority = false) {
		...
		if(!$withPriority) {
			$mitems[] = HTMLHelper::_('select.option', '0', Text::_('COM_JMAP_NOMENUS'), 'value', 'text');
		}
		...
		foreach ( $list as $list_a ) {
			if ($list_a->menutype != $lastMenuType) {
				...
				$mitems [] = HTMLHelper::_ ( 'select.option', '<OPTGROUP>', htmlspecialchars($list_a->title, ENT_COMPAT, 'UTF-8', false) );
			...
			$mitems [] = HTMLHelper::_ ( 'select.option', $list_a->id, $list_a->treename, array( 'option.attr'=>'style', 'attr'=>$styles));
		}
