...
class AdminTools extends CMSPlugin implements SubscriberInterface
{
	...
	/**
	 * AdminTools constructor. Initializes the plugin.
	 *
	 * @param   mixed  $subject
	 * @param   array  $config
	 */
	...
	protected function loadWAFExceptionsSEF()
	{
		 ...
		 */
		$httpHost   = $this->app->input->server->getString('HTTP_HOST', null);
		$scriptName = $this->app->input->server->getString('SCRIPT_NAME', null);
	...
	private function initialize()
	{
		...
		$this->app->bootComponent('com_admintools');
	...
	private function loadFeatures()
	{
		foreach (self::$featureClasses as $className)
		{
			...
			/** @var FeatureBase $o */
			$o = new $className($this->app, $this->db, $this->params, $this->wafConfig, $this->input, $this->blockedRequestHandler, $this->exceptions, $this->skipFiltering, $this);
