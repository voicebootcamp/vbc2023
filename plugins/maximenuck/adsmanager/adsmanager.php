...
class plgMaximenuckAdsmanager extends JPlugin {

	private $type = 'adsmanager';

	private $shallLoad = true;

	function __construct(&$subject, $params) {
		// does not load if the component is not installed
		$this->shallLoad = file_exists(JPATH_SITE . '/administrator/components/com_adsmanager');
		if (! $this->shallLoad)
			return;
	 ...
	 */
	public function onMaximenuckRenderItemAdsmanager($item) {
		require_once(__DIR__ . '/helper/helper_' . $this->type . '.php');
		$adsmanager = MaximenuckHelpersourceAdsmanager::getItems($item->params);
		$html = '<ul class="maximenuck2">';
		foreach ($adsmanager as $article) {
