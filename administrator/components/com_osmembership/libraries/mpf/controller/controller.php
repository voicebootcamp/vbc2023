...
class MPFController
{
	 ...
	 * @throws Exception
	 */
	public static function getInstance($option, MPFInput $input, array $config = [])
	{
		...
		{
			$name = MPFInflector::singularize($input->getCmd('view'));
			if (!$name)
			{
				$name = 'controller';
			}
	...
	public function __construct(MPFInput $input = null, array $config = [])
	{
		...
		$xMethods = get_class_methods('MPFController');
	...
	public function getModel($name = '', array $config = [])
	{
		...
		{
			if (MPFInflector::isPlural($name))
			{
				$config['default_model_class'] = 'MPFModelList';
			}
			else
			{
				if ($this->app->isClient('administrator'))
				{
					$config['default_model_class'] = 'MPFModelAdmin';
				}
				else
				{
					$config['default_model_class'] = 'MPFModelItem';
				}
			}
		}

		//Create model and auto populate model states if required
		$model = MPFModel::getInstance($name, ucfirst($config['class_prefix']) . 'Model', $config);
	 ...
	 *
	 * @return MPFView Reference to the view
	 */
	public function getView($name, $type = 'html', $layout = 'default', array $config = [])
	{
		...
		if (!isset($config['default_view_class']))
		{
			if (MPFInflector::isPlural($name))
			{
				$config['default_view_class'] = 'MPFViewList';
			}
			else
			{
				$config['default_view_class'] = 'MPFViewItem';
			}
		}
		...
		return MPFView::getInstance($name, $type, ucfirst($config['class_prefix']) . 'View', $config);
	}

	/**
	 * Populate model states from controller input
	 *
	 * @param   MPFModel  $model
	 */
	public function populateModelStates($model)
	{
		$states = $model->getState()->getProperties();
		...
		if (count($states))
		{
			...
			else
			{
				foreach ($states as $state)
				{
					$newState = $this->input->get($state, null, 'none');
	 ...
	 *
	 * @return MPFController This object to support chaining.
	 */
	 ...
	 *
	 * @return MPFInput The input object.
	 */
