...
class EventbookingControllerRegistrant extends RADControllerAdmin
{
	...
	public function cancel()
	{
		...
		$this->setRedirect(Route::_('index.php?option=com_eventbooking&view=registrationcancel&id=' . $rowEvent->registrant_id . '&Itemid=' . $this->input->getInt('Itemid',
				0), false));
	}
	...
	public function download_invoice()
	{
		...
		if ($row->user_id == $user->id || $row->email == $user->email || EventbookingHelperAcl::canManageRegistrant($row))
		{
			$canDownload = true;
		}
		else
		...
		{
			$config        = EventbookingHelper::getConfig();
			$invoiceNumber = EventbookingHelper::callOverridableHelperMethod('Helper', 'formatInvoiceNumber',
	...
	public function download_certificate()
	{
		...
		[$fileName, $filePath] = EventbookingHelper::callOverridableHelperMethod('Certificate', 'generateCertificates',
			[[$row], $config]);
	...
	public function download_ticket()
	{
		...
		if (!$row->is_group_billing)
		{
			...
			$ticketFilePaths = EventbookingHelper::callOverridableHelperMethod('Ticket',
				'generateRegistrationTicketsPDF', [$row, $config]);
	...
	public function export()
	{
		...
		[$fields, $headers] = EventbookingHelper::callOverridableHelperMethod('Data', 'prepareRegistrantsExportData',
			[$rows, $config, $rowFields, $fieldValues, $eventId]);

		PluginHelper::importPlugin('eventbooking');

		// Give plugin a chance to process export data
		$results = $this->app->triggerEvent('onBeforeExportDataToXLSX',
		...
		$filePath = EventbookingHelper::callOverridableHelperMethod('Data', 'excelExport',
	...
	public function export_pdf()
	{
		...
		$filePath = EventbookingHelper::callOverridableHelperMethod('Helper', 'generateRegistrantsPDF',
			[$rows, $fields, $headers]);
	...
	protected function getViewListUrl()
	{
		...
		{
			$url = 'index.php?option=com_eventbooking&view=registrants&Itemid=' . EventbookingHelperRoute::findView('registrants',
