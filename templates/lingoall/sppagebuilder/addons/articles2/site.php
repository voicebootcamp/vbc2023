...
class SppagebuilderAddonArticles2 extends SppagebuilderAddons
{
	...
	public function render(){
		...
		if(count((array) $items)) {
			...
			foreach ($items as $key => $item) {
				$output .= '<div class="sppb-col-md-'. round(12/$columns) .'">';
				...
				$output .= '<div class="sppb-article-info-wrap">';
					...
					if($show_author || $show_category || $show_date) {
						...
						if($show_author) {
							...
							$output .= '<span class="sppb-meta-author" itemprop="name"><i class="far fa-user-circle"></i> ' . $author . '</span>';
						}

						if($show_date) {
							$output .= '<span class="sppb-meta-date" itemprop="datePublished"><i class="far fa-calendar-alt"></i> ' . Jhtml::_('date', $item->publish_up, 'DATE_FORMAT_LC3') . '</span>';
						}
						...
						if($show_category) {
							...
							$output .= '<span class="sppb-meta-category"><i class="far fa-folder"></i> <a href="'. $item->catUrl .'" itemprop="genre">' . $item->category . '</a></span>';
						}
