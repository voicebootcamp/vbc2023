...
class EventbookingModelCommonRegistrant extends RADModelAdmin
{
	...
	protected function addNewRegistration($row, $input, $form)
	{
		...
		if ($row->total_amount == 0)
		{
			...
			{
				$fees = EventbookingHelper::callOverridableHelperMethod('Registration',
					'calculateIndividualRegistrationFees', [$event, $form, $data, $config, $row->payment_method],
					'Helper');
			...
			{
				$rate               = EventbookingHelper::callOverridableHelperMethod('Registration',
					'getRegistrationRate', [$data['event_id'], $data['number_registrants']]);
				$feeCalculationTags = [
					...
					&& file_exists(JPATH_ROOT . '/components/com_eventbooking/fields.xml'))
				{
					...
					{
						$feeCalculationTags[strtoupper($customFieldName)] = $filterInput->clean($param['value'],
							'float');
					}
				}
	...
	protected function updateRegistration($row, $input, $form)
	{
		...
		$config->collect_member_information = EventbookingHelperRegistration::isCollectMembersInformation($event,
			$config);
		...
		elseif ($row->published == 2 && $published != 2)
		{
			...
			{
				EventbookingHelper::callOverridableHelperMethod('Mail', 'sendWaitingListNotificationEmail',
					[$row, $config]);
			}
		}
	}
	...
	protected function recalculateRegistrationFees($row, &$data, $form)
	{
		...
		{
			$fees = EventbookingHelper::callOverridableHelperMethod('Registration',
				'calculateIndividualRegistrationFees', [$event, $form, $data, $config, $row->payment_method], 'Helper');
		}
		elseif ($row->is_group_billing)
		{
			...
			$fees = EventbookingHelper::callOverridableHelperMethod('Registration', 'calculateGroupRegistrationFees',
				[$event, $form, $data, $config, $row->payment_method], 'Helper');
		}
		else
		{
		...
		if ($state == 1 && count($cid))
		{
			...
			foreach ($cid as $registrantId)
			{
				...
				if (in_array($row->published, [0, 3]))
				{
					...
					EventbookingHelper::callOverridableHelperMethod('Mail', 'sendRegistrationApprovedEmail',
						[$row, $config]);
				}
			}
	...
	private function storeRegistrantTickets($row, $data)
	{
		...
		if (!$event->has_multiple_ticket_types || !$user->authorise('eventbooking.registrantsmanagement',
				'com_eventbooking'))
		{
			return;
		}
	...
	public function cancelRegistrations($cid = [])
	{
		...
		foreach ($cid as $id)
		{
			...
			if ($row->group_id > 0)
			{
				$app->enqueueMessage(Text::sprintf('Cancelling group member with ID %s is not supported. You need to cancel registration for whole group',
					$row->id));
	...
	public function batchMail($input)
	{
		...
		foreach ($rows as $row)
		{
			...
			if (!isset($loadedEvents[$language . '.' . $row->event_id]))
			{
				...
				if ($fieldSuffix)
				{
					EventbookingHelperDatabase::getMultilingualFields($query,
						['title', 'short_description', 'description', 'price_text'], $fieldSuffix);
				}
			...
			$replaces                = EventbookingHelperRegistration::getRegistrationReplaces($row, $event,
				$row->user_id, $config->multiple_booking);
			...
			$message = EventbookingHelperHtml::loadSharedLayout('emailtemplates/tmpl/email.php',
	...
	public function batchSMS($cid, $message)
	{
		...
		Factory::getApplication()->triggerEvent('onEBSendingSMSReminder', [$rows]);
	}
