...
if (count ( $this->source->data ) != 0) {  
	foreach ( $this->source->data as $index=>$elm ) {
		...
		if (preg_match('/[\x80-\xFF]/', $elm->link)) {
			$elm->link = implode('/', array_map('rawurlencode', explode('/', $elm->link)));
		}
?>
<item>
<title><?php echo htmlspecialchars($elm->title, ENT_COMPAT, 'UTF-8'); ?></title>
<link><?php echo str_replace(' ', '%20', $this->liveSite . htmlspecialchars($elm->link, ENT_NOQUOTES, 'UTF-8', false)); ?></link>
