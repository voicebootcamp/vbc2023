...
class HtmLawed {
	public static function executeHtmlLawed($t, $C=1, $S=array()) {
...
function htmLawed($t, $C=1, $S=array())
{
  ...
  if ($C['cdata'] || $C['comment']) {
    $t = preg_replace_callback('`<!(?:(?:--.*?--)|(?:\[CDATA\[.*?\]\]))>`sm', '\JExtstore\Component\JMap\Administrator\Framework\AIGenerator\hl_commentCdata', $t);
  }
  $t =
    preg_replace_callback(
      '`&amp;([a-zA-Z][a-zA-Z0-9]{1,30}|#(?:[0-9]{1,8}|[Xx][0-9A-Fa-f]{1,7}));`',
      '\JExtstore\Component\JMap\Administrator\Framework\AIGenerator\hl_entity',
      str_replace('&', '&amp;', $t));
  ...
  $t = preg_replace_callback('`<(?:(?:\s|$)|(?:[^>]*(?:>|$)))|>`m', '\JExtstore\Component\JMap\Administrator\Framework\AIGenerator\hl_tag', $t);
...
function hl_spec($t)
{
  ...
  $t =
    str_replace(
      array("\t", "\r", "\n", ' '),
      '',
      preg_replace_callback('/"(?>(`.|[^"])*)"/sm', '\JExtstore\Component\JMap\Administrator\Framework\AIGenerator\hl_aux1', trim($t)));
      ...
      if ($attr == 'style' && !$C['style_pass']) {
        ...
        $v =
          preg_replace_callback(
            ...
            '\JExtstore\Component\JMap\Administrator\Framework\AIGenerator\hl_url',
            $v);
        $v = !$C['css_expression']
  ...
  $t =
    preg_replace(
      ...
      preg_replace_callback(
        ...
        '\JExtstore\Component\JMap\Administrator\Framework\AIGenerator\hl_aux2',
        $t));
