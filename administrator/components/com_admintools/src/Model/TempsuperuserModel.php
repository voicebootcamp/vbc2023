...
use Akeeba\Component\AdminTools\Administrator\Table\Mixin\NoSuperUsersCheckFlags;
use Exception;
...
class TempsuperuserModel extends AdminModel
{
	...
	public function save($data)
	{
		...
		try
		{
			...
			try
			{
				if ($isNew)
				{
					$this->getDbo()->insertObject($table->getTableName(), $table, $table->getKeyName());
				}
				else
				{
					$this->getDbo()->updateObject($table->getTableName(), $table, $table->getKeyName(), true);
				}
			}
			catch (Exception $e)
			{
	...
	private function generatePassword(int $length = 64, ?int $minInt = 0, ?int $minSymbol = 0, ?int $minUpper = 0, ?int $minLower = 0): string
	{
		...
		$remainingLength = $length - $minLength;
