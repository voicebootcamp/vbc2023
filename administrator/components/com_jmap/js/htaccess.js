...
(function($) {
	var Htaccess = function() {
		 ...
		 * @var Object
		 */
		var validationSnippet = '<ul class="errorlist"><li class="validation badge bg-danger">' + COM_JMAP_HTACCESS_REQUIRED + '</li></ul>';
		...
		(function __construct() {
			// Popover trigger
			[].slice.call(document.querySelectorAll('label.hasHtaccessPopover')).map(function (popoverEl) {
			...
			// Add desc popover
			[].slice.call(document.querySelectorAll('label.hasrightPopover')).map(function (popoverEl) {
				return new bootstrap.Popover(popoverEl,{
					template : '<div class="popover"><div class="popover-arrow"></div><h3 class="popover-header"></h3><div class="popover-body"></div></div>',
					trigger:'hover',
					placement:'right',
