...
use JExtstore\Component\JMap\Administrator\Framework\Helpers\Error as BaseError;
use JExtstore\Component\JMap\Administrator\Framework\File as JMapFile;
 ...
 * @since 3.0
 */
class Sources extends BaseError {
	/**
	 * Database connector
	 * 
	 * @access private
	...
	public function import() {
		...
		try {
			if(!$tmpFile || !$tmpFileName) {
				throw new JMapException(Text::_('COM_JMAP_NOFILE_SELECTED'), 'error');
			}
			
			$tmpFileSize = $file['size'];
			$allowedFileSize = 2 * 1024 * 1024; // MB->Bytes
			if($tmpFileSize > $allowedFileSize) {
				throw new JMapException(Text::_('COM_JMAP_SIZE_ERROR') .' Max 2MB.', 'error');
			}
			
			$tmpFileExtension = @array_pop(explode('.', $tmpFileName));
			if($tmpFileExtension != 'json') {
				throw new JMapException(Text::_('COM_JMAP_EXT_ERROR'), 'error');
			}
			...
			if(!is_array($objectToRestore)) {
				throw new JMapException(Text::_('COM_JMAP_INVALID_IMPORT_DATA'), 'error');
			}
			...
			foreach ($objectToRestore as $dataSource) {
				...
				if($dataSource->type == 'plugin' && !Folder::exists(JPATH_COMPONENT_ADMINISTRATOR . '/plugins/' . strtolower($dataSource->name))) {
					$this->app->enqueueMessage(Text::sprintf('COM_JMAP_NOPLUGIN_INSTALLED_IMPORTED_DATASOURCE', $dataSource->name));
					continue;
				}
			...
			if(!count($dbQueryArray)) {
				throw new JMapException(Text::_('COM_JMAP_NO_IMPORT_DATA_FOUND'), 'warning');
			}
			...
			// Reorder post insert
			$table = $this->app->bootComponent('com_jmap')->getMVCFactory()->createTable('Sources', 'Administrator');
			if (! $table->reorder ()) {
	...
	public function export($ids = null) { 
		...
		try {
			...
			if(!$resultInfo) {
				throw new JMapException(Text::_('COM_JMAP_ERROR_NODATA_TOEXPORT'), 'error');
			}
	...
	public function install() {
		...
		try {
			...
			if($tmpFileExtension != 'zip') {
				throw new JMapException(Text::_('COM_JMAP_EXT_PLUGIN_ERROR'), 'error');
			}
			...
			if(Folder::exists($folderPluginPath)) {
				Folder::delete($folderPluginPath);
			}
			Folder::create($folderPluginPath);

			// Copy the zip archive to the target directory
			if(!move_uploaded_file($tmpFile, $folderPluginPath . '/' . $tmpFileName)) {
				throw new JMapException(Text::_('COM_JMAP_UPLOAD_ERROR'), 'error');
			}

			// Unpack the zip archive
			$archiveManager = new JArchive();
			$archiveManager->extract($folderPluginPath . '/' . $tmpFileName, $folderPluginPath);

			// Delete the original zip archive
			JMapFile::delete($folderPluginPath . '/' . $tmpFileName);

			// Parse the XML manifest file of the plugin
			$manifestFiles = glob( $folderPluginPath . '/*.xml');
			if(!count($manifestFiles)) {
				throw new JMapException(Text::_('COM_JMAP_ERROR_MANIFEST_NOTFOUND'), 'error');
			}
			...
			$this->requestArray['name'] = ucfirst($pluginName);
			$this->requestArray['description'] = (string)$pluginDescription;
			foreach ($pluginParams as $field) {
				...
				$this->requestArray['params'][$fieldName] = $fieldValue;
			}
			...
			if($folderPluginName != strtolower($pluginName)) {
				...
				if(Folder::exists(JPATH_COMPONENT_ADMINISTRATOR . '/plugins/' . strtolower($pluginName))) {
					Folder::delete(JPATH_COMPONENT_ADMINISTRATOR . '/plugins/' . strtolower($pluginName));
				}
