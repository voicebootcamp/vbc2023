		...
		?>
			...
			<thead>
				<tr>
					...
					<?php
						...
						if ($this->config->get('history_show_registration_date', 1))
						{
							$cols++;
						 ?>
	                        <th class="list_event_date">
						...
						if ($this->config->get('history_show_transaction_id', 0))
						{
							$cols++;
						?>
	                        <th class="eb-transaction-id-column">
	                            <?php echo HTMLHelper::_('grid.sort', Text::_('EB_TRANSACTION_ID'), 'tbl.transaction_id', $this->lists['order_Dir'], $this->lists['order']); ?>
	                        </th>
	                    <?php
						}

						if ($this->config->get('history_show_registration_status', 1))
						{
							$cols++;
						?>
	                        <th class="list_id">
			...
			for ($i = 0, $n = count($this->items); $i < $n; $i++)
			{
				...
				<tr>
					...
					if ($this->config->get('history_show_transaction_id'))
					{
					?>
	                    <td class="tdno<?php echo $i; ?> center" data-content="<?php echo Text::_('EB_TRANSACTION_ID'); ?>">
	                ...
	                <td class="tdno<?php echo $i; ?> center" data-content="<?php echo Text::_('EB_REGISTRATION_STATUS'); ?>">
						...
						switch($row->published)
						{
							case 0 :
								...
								if ($hasOnlinePaymentMethods && strpos($row->payment_method, 'os_offline') !== false)
								{
								?>
							...
							case 3:
								...
								if ($this->config->enable_waiting_list_payment && $row->group_id == 0 && $row->amount > 0)
								{
