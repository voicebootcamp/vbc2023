...
?>
<video:video>
<video:thumbnail_loc><?php echo htmlspecialchars(preg_match('/^http|^\/\//i', $videoPoster) ? $videoPoster : $this->liveSite . '/' . ltrim($videoPoster, '/'), ENT_NOQUOTES, 'UTF-8', false);?></video:thumbnail_loc>
<video:title><?php echo htmlspecialchars($videoTitle, ENT_COMPAT, 'UTF-8');?></video:title>
<video:description><![CDATA[<?php echo StringHelper::substr($videoDescription, 0, 2048);?>]]></video:description>
<?php 	
			...
			foreach ($childSources as $source) {
				...
				if($source->nodeName == 'source' && $source->hasAttributes()) {
					...
					$videoSrc = htmlspecialchars(preg_match('/^http|^\/\//i', $videoSrc) ? $videoSrc : $this->liveSite . '/' . ltrim($videoSrc, '/'), ENT_NOQUOTES, 'UTF-8', false)
?>
