...
{
	use EventbookingControllerCaptcha, RADControllerDownload;
	...
	public function add()
	{
		[$Itemid, $layout] = EventbookingHelper::getAddEditEventFormLayout();
	...
	public function edit()
	{
		...
		[, $layout] = EventbookingHelper::getAddEditEventFormLayout();
		$this->input->set('view', 'event');
	...
	public function save()
	{
		...
		if ($task == 'save2copy')
		{
			...
			$return = Route::_('index.php?option=com_eventbooking&view=event&id=' . $this->input->getInt('id', 0) . '&layout=' . $formLayout . '&Itemid=' . $this->input->getInt('Itemid'), false);
	...
	public function send_invite()
	{
		...
		if ($config->show_invite_friend)
		{
			...
			$this->setRedirect(
				Route::_(
					'index.php?option=com_eventbooking&view=invite&layout=complete&tmpl=component&Itemid=' . $this->input->getInt('Itemid', 0),
					false
				)
			);
		}
	...
	public function download_ical()
	{
		...
		if ($eventId)
		{
			...
			$fileName = JPATH_ROOT . '/media/com_eventbooking/icsfiles/' . $eventId . '.ics';
			EventbookingHelper::generateIcs([$event], $fromEmail, $fromName, $fileName);

			$this->processDownloadFile($fileName, ApplicationHelper::stringURLSafe($event->title) . '.ics');
		}
	}
