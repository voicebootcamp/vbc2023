...
defined('_JEXEC') or die('Restricted access');
	...
	try{
		...
		logIt($e->getMessage(), "wiz_paypal_return_tmpl_default", "", "");
	...
	$database = JFactory::getDBO();
	$sql = 'SELECT * FROM #__sv_apptpro3_paypal_transactions WHERE custom = "'.$database->escape($req_id).
	...
	if(sv_count_($transactions) == 0){
		if($cart == "yes"){
			...
			$message = buildMessage($jinput->getString('custom'), "in_progress", "Yes");
		}
	} else {	
		if($cart == "yes"){
			// used cart confirmation stored in session
