...
class admin_detailModeluser_credit_detail extends JModelLegacy
{
	...
	function _loadData()
	{
		...
		if (empty($this->_data))
		{
			$query = 'SELECT * FROM '.$this->_table_prefix.'user_credit WHERE id_user_credit = '. $this->getDbo()->escape($this->_id_user_credit);
	...
	function store($data)
	{
		...
		$sql = "SELECT count(*) as dupes FROM #__sv_apptpro3_user_credit WHERE gift_cert = '".$this->getDbo()->escape($row->gift_cert)."' AND id_user_credit != ".$this->getDbo()->escape($row->id_user_credit);
		try{
	...
	function publish($cid = array(), $publish = 1)
	{
		...
		if (sv_count_($cid ))
		{
			...
			$query = 'UPDATE '.$this->_table_prefix.'user_credit'
				...
				. ' WHERE id_user_credit IN ( '.$this->getDbo()->escape($cids).' )'
  ...
  //second parameter: condition
	...
	function delete($cid = array())
	{
		...
		{
			$cids = implode( ',', $cid );
			$query = "DELETE FROM #__sv_apptpro3_user_credit_activity WHERE user_id IN (SELECT user_id FROM #__sv_apptpro3_user_credit WHERE id_user_credit IN (".$this->getDbo()->escape($cids)."))";
			$this->_db->setQuery($query);
			if (!$this->_db->execute()) {
				$this->setError($row->getError());
				return false;
			}
			$query = 'DELETE FROM '.$this->_table_prefix.'user_credit WHERE id_user_credit IN ( '.$this->getDbo()->escape($cids).' )';
			$this->_db->setQuery( $query );
			if(!$this->_db->execute()) {
				$this->setError($row->getError());
				return false;
	...
	function _loadCredit_usage_info()
	{
		...
		$sql = "SELECT #__sv_apptpro3_user_credit_activity.*, #__users.name as operator, #__sv_apptpro3_requests.startdate, ".
			...
			" (SELECT user_id FROM #__sv_apptpro3_user_credit WHERE id_user_credit = ".$this->getDbo()->escape($this->_id_user_credit).") ORDER BY stamp desc";
		//echo $sql;		
		$this->_db->setQuery($sql);
