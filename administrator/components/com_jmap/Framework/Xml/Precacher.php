<?php
namespace JExtstore\Component\JMap\Administrator\Framework\Xml;
/**
 *
...
use Joomla\CMS\Filesystem\Stream as FilesystemStream;
use Joomla\CMS\Date\Date;
use JExtstore\Component\JMap\Administrator\Framework\Exception\ExceptionPrecaching as JMapExceptionPrecaching;
 ...
 * @since 2.3
 */
class Precacher implements IJMapXmlPrecacher {
	/**
	...
	protected function writeFile($data) {
		...
		if($this->processStatus == 'start' && file_exists($tempFileName)) {
			if(!File::delete($tempFileName)) {
				throw new JMapExceptionPrecaching ( Text::_ ( 'COM_JMAP_PRECACHING_ERROR_DELETE_TEMPFILE'), 'error', 'delete_temp_file' );
			}
		}
		
		// Open file in write append mode
		if (! $this->fileStreamWriter->open ( $tempFileName, 'a' )) {
			throw new JMapExceptionPrecaching ( Text::sprintf ( 'COM_JMAP_PRECACHING_ERROR_OPENING_FILE', $this->fileStreamWriter->getError () ), 'error', 'open_file' );
		}
		...
		if ($data) {
			...
			if (! $result) {
				throw new JMapExceptionPrecaching ( Text::sprintf ( 'COM_JMAP_PRECACHING_ERROR_WRITING_FILE', $this->fileStreamWriter->getError () ), 'error', 'write_file' );
			}
		}
		
		// Finished operation, close file handle
		if (! $this->fileStreamWriter->close ()) {
			throw new JMapExceptionPrecaching ( Text::sprintf ( 'COM_JMAP_PRECACHING_ERROR_CLOSING_FILE', $this->fileStreamWriter->getError () ), 'error', 'close_file' );
		}
		...
		if ($this->processStatus == 'end') {
			if (! rename ( $tempFileName, $this->preCachingPath . $this->fileName )) {
				throw new JMapExceptionPrecaching ( Text::_ ( 'COM_JMAP_PRECACHING_ERROR_RENAMING_FILE' ), 'error', 'rename_file' );
			}
			// Set filemtime
	 ...
	 * @return Object&
	 */
	public function __construct($fileName, FilesystemStream $fileWriter) {
