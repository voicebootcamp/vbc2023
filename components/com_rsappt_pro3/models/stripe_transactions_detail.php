...
class admin_detailModelstripe_transactions_detail extends JModelLegacy
{
	...
	function _loadData()
	{
		...
		if (empty($this->_data))
		{
			$query = 'SELECT * FROM '.$this->_table_prefix.'stripe_transactions WHERE id_stripe_transactions = '. $this->getDbo()->escape($this->_id_stripe_transactions);
	...
	function store($data)
	{
		...
		if (!$row->id_stripe_transactions) {
			$where = 'id_stripe_transactions = ' . $this->getDbo()->escape($row->id_stripe_transactions) ;
...
/*		if (!$row->check()) {
	...
	function delete($cid = array())
	{
		...
		{
			$cids = implode( ',', $cid );
			$query = 'DELETE FROM '.$this->_table_prefix.'stripe_transactions WHERE id_stripe_transactions IN ( '.$this->getDbo()->escape($cids).' )';
			$this->_db->setQuery( $query );
			if(!$this->_db->execute()) {
				$this->setError($row->getError());
				return false;
