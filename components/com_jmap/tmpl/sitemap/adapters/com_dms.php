...
switch ($targetViewName) {
	case 'document':
		...
		if(strpos($seflink, 'component/dms')) {
			...
			$seflink = \JMapRoute::_ ($helperRouteClass::$classMethod($elm->id, $elm->catid, $fallbackItemid));
		}
	...
	case 'category':
		...
		if(strpos($seflink, 'component/dms')) {
			$needles = array('categories'=>array(0));
			$fallbackItemid = $helperRouteClass::findItem($needles);
			$seflink = \JMapRoute::_ ($helperRouteClass::$classMethod($elm->id, $fallbackItemid));
		}
		break;
}	
