...
class resources_detailModelresources_detail extends JModelLegacy
{
	...
	function _initData()
	{
		...
		if (empty($this->_data))
		{
			...
			$detail->res_stripe_sk = "";	
			$detail->res_stripe_pk = "";	
	...
	function store($data)
	{
		...
		if (!$row->id_resources) {			
			...
			$row->id_resources = $this->getState('resources.id_resources');			
		} 
	...
	function publish($cid = array(), $publish = 1)
	{
		...
		if (sv_count_($cid ))
		{
			...
			$query = 'UPDATE '.$this->_table_prefix.'resources'
				...
				. ' AND ( checked_out = 0 OR ISNULL(checked_out) OR ISNULL(checked_out) OR ( checked_out = ' .$user->get('id'). ' ) )'
			;
//echo $query;
//exit;
			$this->_db->setQuery( $query );
