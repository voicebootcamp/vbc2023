			...
			foreach ($this->items as $i => $item) :
				...
				<li class="clearfix" data-alias="<?php echo $item->alias ?>" data-level="<?php echo $item->level ?>" data-id="<?php echo (int) $item->id; ?>" data-parent="<?php echo (int) $item->parent_id; ?>" data-newcol="<?php echo $item->params->get('maximenu_createcolumn', 0) ?>" data-colwidth="<?php echo $item->params->get('maximenu_colwidth', '180') ?>" data-home="<?php echo $item->home ?>">
					<div class="itemwrapper">
						...
						<div>
							<span class="ckmove ckaction"><i class="fas fa-arrows-alt"></i></span>
							<span class="ckbutton-group">
								...
								<?php if ($canEdit) : ?>
									<span class="ckdescfield">
										<input class="cktip" type="text" onblur="ckSaveDescription(this)" title="<?php echo JText::_('COM_MAXIMENUCK_ITEM_DESCRIPTION'); ?>" data-id="<?php echo (int) $item->id; ?>" data-text-origin="<?php echo addslashes($item->params->get('maximenu_desc', '')); ?>" value="<?php echo addslashes($item->params->get('maximenu_desc', '')); ?>" />
									</span>
								<?php else : ?>
									<?php echo addslashes($item->params->get('maximenu_desc', '')); ?>
								<?php endif; ?>

								<span class="ckbutton-group columnbuttons">
									<a class="cktip ckbutton createcolumn<?php if ($newcol) echo ' active'; ?>" title="<?php echo JText::_('COM_MAXIMENUCK_CREATE_COLUMN'); ?>" onclick="ckToggleColumn(this)">
										<i class="fas fa-columns"></i>
									</a>
									<a class="cktip ckbutton colwidth btnvalue" title="<?php echo JText::_('COM_MAXIMENUCK_COLUMN_WIDTH'); ?>" onclick="ckChangeColWidth(this)"><?php echo $item->params->get('maximenu_colwidth', '180'); ?></a>
									
									<a class="cktip ckbutton createnewrow<?php if ($newrow) echo ' active'; ?>" title="<?php echo JText::_('COM_MAXIMENUCK_CREATE_ROW'); ?>" onclick="ckCreateNewRow(this)">
										<i class="fas fa-grip-lines"></i>
									</a>
								</span>
								<span class="ckbutton-group submenuwidthbuttons">
									<a class="cktip ckbutton submenuwidth btnvalue <?php echo ($subwidth ? 'ckhastext' : ''); ?>" title="<?php echo JText::_('COM_MAXIMENUCK_SUBMENUWIDTH'); ?>" onclick="ckChangeSubmenuWidth(this)"><i class="fas fa-arrows-alt-h"></i><span class="valuetxt"><?php echo $subwidth; ?></span></a>
									<a class="cktip ckbutton submenuheight btnvalue <?php echo ($subheight ? 'ckhastext' : ''); ?>" title="<?php echo JText::_('COM_MAXIMENUCK_SUBMENUHEIGHT'); ?>" onclick="ckChangeSubmenuHeight(this)"><i class="fas fa-arrows-alt-v"></i><span class="valuetxt"><?php echo $subheight; ?></span></a>
									<a class="cktip ckbutton submenuleftmargin btnvalue <?php echo ($subleft ? 'ckhastext' : ''); ?>" title="<?php echo JText::_('COM_MAXIMENUCK_SUBMENULEFTMARGIN'); ?>" onclick="ckChangeSubmenuLeftMargin(this)"><i class="fas fa-angle-double-right"></i><span class="valuetxt"><?php echo $subleft; ?></span></a>
									<a class="cktip ckbutton submenutopmargin btnvalue <?php echo ($subtop ? 'ckhastext' : ''); ?>" title="<?php echo JText::_('COM_MAXIMENUCK_SUBMENUTOPMARGIN'); ?>" onclick="ckChangeSubmenuTopMargin(this)"><i class="fas fa-angle-double-down"></i><span class="valuetxt"><?php echo $subtop; ?></span></a>
									<a class="cktip ckbutton fullwidth<?php if (stristr($item->params->get('maximenu_liclass', ''), 'fullwidth')) echo ' active ckbutton-primary'; ?>" title="<?php echo JText::_('COM_MAXIMENUCK_FULLWIDTH'); ?>" onclick="ckChangeFullwidthClass(this)"><i class="fas fa-expand"></i></a>
								</span>
								<span class="ckbutton-group togglemobileoptions">
									<span class="togglescreen ckbutton cktip <?php echo ($item->params->get('maximenu_disabledesktop', '0') == '1' ? 'disable' : '') ?>" title="<?php echo JText::_('COM_MAXIMENUCK_ENABLE_DESKTOP'); ?>" onclick="ckToggleDesktopState(this)"><span class="fas fa-times"></span><i class="fas fa-desktop"></i></span>
									<span class="togglemobile ckbutton cktip <?php echo ($item->params->get('maximenu_disablemobile', '0') == '1' ? 'disable' : '') ?>" title="<?php echo JText::_('COM_MAXIMENUCK_ENABLE_MOBILE'); ?>" onclick="ckToggleMobileState(this)"><span class="fas fa-times"></span><i class="fas fa-mobile-alt"></i></span>
								</span>
								...
								<span class="ckbutton-group iconoptions">
									<span id="iconselect<?php echo $item->id ?>" class="iconselect ckbutton cktip <?php echo ($item->params->get('maximenu_icon','') ? 'active' : '') ?>" title="<?php echo JText::_('COM_MAXIMENUCK_ITEM_ICON_DESC'); ?>" onclick="ckSelectIcon(this.id)"><?php echo JText::_('COM_MAXIMENUCK_ITEM_ICON'); ?>&nbsp;<span class="<?php echo $item->params->get('maximenu_icon','') ?>"></span></span>
								...
								<span class="moduleoptions ckbutton-group">
									<span id="moduleselect<?php echo $item->id ?>" class="moduleselect ckbutton cktip <?php echo $modulebtnclass ?> <?php echo ($modulename ? 'ckhastext' : ''); ?>" title="<?php echo JText::_('COM_MAXIMENUCK_ITEM_MODULE'); ?>" onclick="ckSelectModule(this.id)"><i class="fas fa-database"></i><span class="modulename"><?php echo $modulename; ?></span></span>
