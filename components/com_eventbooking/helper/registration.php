...
class EventbookingHelperRegistration
{
			...
			if (count($quantityFields))
			{
				foreach ($quantityFields as $field)
				{
					...
					if (count($values) && count($quantityValues))
					{
						...
						$values = EventbookingHelper::callOverridableHelperMethod('html', 'getAvailableQuantityOptions',
							[&$values, &$quantityValues, $event->id, $field->id, $isMultiple, $multilingualValues]);
	...
	public static function getAllEventFields($eventId)
	{
		...
		if ($eventId)
		{
			...
			if ($config->custom_field_by_category)
			{
				...
				$query->where('(category_id = -1 OR id IN (SELECT field_id FROM #__eb_field_categories WHERE category_id IN (' . implode(',',
						$categoryIds) . ')))');
			}
			else
			{
	...
	public static function getDepositPaymentFormFields()
	{
		...
		if ($fieldSuffix)
		{
			EventbookingHelperDatabase::getMultilingualFields($query, ['title', 'description', 'values', 'default_values', 'depend_on_options'],
				$fieldSuffix);
		}
		...
		if (empty($cache[$cacheKey]))
		{
			...
			if ($fieldSuffix)
			{
				EventbookingHelperDatabase::getMultilingualFields($query,
					['title', 'place_holder', 'description', 'values', 'default_values', 'depend_on_options'], $fieldSuffix);
			}
			...
			if ($registrationType == 4)
			{
				...
				if ($config->custom_field_by_category)
				{
					...
					$query->where('(category_id = -1 OR id IN (SELECT field_id FROM #__eb_field_categories WHERE category_id IN (' . $subQuery . ')))');
				}
				else
				{
					if (!count($items))
					...
					$query->where('(event_id = -1 OR id IN (SELECT field_id FROM #__eb_field_events WHERE event_id IN (' . implode(',',
							$items) . ')))');
				}
	...
	public static function getRegistrationReplaces($row, $rowEvent = null, $userId = 0, $enableShoppingCart = false, $loadCss = true)
	{
		...
		$replaces = EventbookingHelper::callOverridableHelperMethod('Registration', 'buildTags', [$row, $form, $rowEvent, $config, $loadCss],
			'Helper');
	...
	public static function calculateRegistrationFees($row, $paymentMethod)
	{
		...
		$fees['payment_processing_fee'] = EventbookingHelper::callOverridableHelperMethod('Registration', 'calculatePaymentProcessingFee',
	...
	public static function calculateIndividualRegistrationFees($event, $form, $data, $config, $paymentMethod = null)
	{
		...
		[$vatNumberValid, $taxRate, $showVatNumberField] = EventbookingHelper::callOverridableHelperMethod('Registration',
			'calculateRegistrationTaxRate', [$event, $form, $data, $config]);
		...
		if ($user->id)
		{
			$discountRate = EventbookingHelper::callOverridableHelperMethod('Registration', 'calculateMemberDiscount',
				[$event->discount_amounts, $event->discount_groups]);
	...
	public static function calculateGroupRegistrationFees($event, $form, $data, $config, $paymentMethod = null)
	{
		...
		if ($event->tax_rate > 0 && ($totalAmount - $discountAmount + $lateFee > 0))
		{
			...
			if ($collectMemberInformation)
			{
				for ($i = 0; $i < $numberRegistrants; $i++)
				{
					$membersTaxAmount[$i] = round(($membersTotalAmount[$i] - $membersDiscountAmount[$i] + $membersLateFee[$i]) * $event->tax_rate / 100,
						2);
		...
		for ($i = 0, $n = count($items); $i < $n; $i++)
		{
			...
			if ($event->tax_rate > 0)
			{
				...
				if ($config->collect_member_information_in_cart)
				{
					for ($j = 0; $j < $quantity; $j++)
					{
						$membersTaxAmount[$eventId][$j] = round($event->tax_rate * ($membersTotalAmount[$eventId][$j] - $membersDiscountAmount[$eventId][$j] + $membersLateFee[$eventId][$j]) / 100,
							$numberDecimals);
			...
			if ($registrantAmount > 0)
			{
				...
				{
					$registrantPaymentProcessingFee = EventbookingHelper::callOverridableHelperMethod('Registration', 'calculatePaymentProcessingFee',
	...
	public static function saveRegistration($data)
	{
		...
		{
			return EventbookingHelper::callOverridableHelperMethod('Registration', 'userRegistrationCB',
	 ...
	 * @return stdClass
	 */
	public static function getGroupMemberTicketTypeData($groupMemberId)
	{
		$db    = Factory::getDbo();
		$query = $db->getQuery(true);
		$query->select('a.*')
	...
	public static function updateGroupRegistrationRecord($groupId)
	{
		...
		if (EventbookingHelperRegistration::isCollectMembersInformation($event, $config))
		{
			$db    = Factory::getDbo();
			$query = $db->getQuery(true)
	...
	public static function buildDepositPaymentTags($row, $config)
	{
		...
		$replaces['PAYMENT_AMOUNT']  = EventbookingHelper::formatCurrency($row->amount - $row->deposit_amount + $row->deposit_payment_processing_fee,
	...
	public static function buildEventTags($event, $config, $row = null, $Itemid = 0)
	{
		...
		$replaces['event_link']    = Route::link('site', EventbookingHelperRoute::getEventRoute($event->id, $event->main_category_id, $defaultMenuId),
			false, 0, true);
		$replaces['category_link'] = Route::link('site', EventbookingHelperRoute::getCategoryRoute($event->main_category_id, $defaultMenuId), false,
			0, true);
	...
	public static function buildTags($row, $form, $event, $config, $loadCss = true)
	{
		...
		if ($config->multiple_booking)
		{
			...
			$replaces['total_amount_minus_discount'] = EventbookingHelper::formatCurrency($totalAmount - $discountAmount, $config,
		...
		else
		{
			...
			$replaces['total_amount_minus_discount'] = EventbookingHelper::formatCurrency($row->total_amount - $row->discount_amount, $config,
				$event->currency_symbol);
			...
			$replaces['payment_processing_fee']      = EventbookingHelper::formatCurrency($row->payment_processing_fee, $config,
				$event->currency_symbol);
			...
			if ($event->has_multiple_ticket_types)
			{
				...
				$db->setQuery($query);
				$replaces['TICKET_TYPES_TABLE'] = EventbookingHelperHtml::loadSharedLayout('emailtemplates/tickettypes.php',
					['ticketTypes' => $db->loadObjectList(), 'eventId' => $row->event_id]);
			}
		}

		$rate                          = EventbookingHelper::callOverridableHelperMethod('Registration', 'getRegistrationRate',
		...
		$replaces['current_month']        = HTMLHelper::_('date', 'Now', 'F');
		$replaces['current_month_short']  = HTMLHelper::_('date', 'Now', 'M');
		...
		if ((int) $row->register_date)
		{
			...
			$replaces['register_date_month']        = HTMLHelper::_('date', $row->register_date, 'F');
			$replaces['register_date_month_short']  = HTMLHelper::_('date', $row->register_date, 'M');
		}
		else
		{
		...
		if ($row->is_group_billing && $collectMemberInformation)
		{
			...
			$replaces['group_members'] = EventbookingHelperHtml::loadSharedLayout('emailtemplates/tmpl/email_group_members.php',
	...
	public static function getEmailContent($config, $row, $loadCss = true, $form = null, $toAdmin = false)
	{
		...
		else
		{
			...
			$collectMemberInformation = self::isCollectMembersInformation($rowEvent, $config);
	...
	public static function generateQrcode($registrantId)
	{
		...
		static::generateQRCODEFile($checkinUrl, JPATH_ROOT . '/media/com_eventbooking/qrcodes/' . $registrantId . '.png',
	...
	{
		// File was generated before, do not need to generate again
