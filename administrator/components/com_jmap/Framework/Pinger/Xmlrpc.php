<?php
namespace JExtstore\Component\JMap\Administrator\Framework\Pinger\Xmlrpc;
/**
...
class Client
{
		...
		function& send($msg, $timeout=10, $method='')
		{
			...
			if(is_array($msg))
			{
				// $msg is an array of \JExtstore\Component\JMap\Administrator\Framework\Pinger\Xmlrpc\Message's
		...
		*/
			...
			$proxyport=0, $proxyusername='', $proxypassword='', $proxyauthtype=1)
		{
			...
			else
			{
				...
				$r=new Response(0, $GLOBALS['xmlrpcerr']['http_error'], $this->errstr . ' (' . $this->errno . ')');
				return $r;
			}
			...
			if(!fputs($fp, $op, strlen($op)))
			{
				...
				$r=new Response(0, $GLOBALS['xmlrpcerr']['http_error'], $this->errstr);
				return $r;
			}
			else
			{
		...
		*/
			...
			$keepalive=false, $key='', $keypass='')
		{
			if(!function_exists('curl_init'))
			{
				...
				$r=new Response(0, $GLOBALS['xmlrpcerr']['no_curl'], $GLOBALS['xmlrpcstr']['no_curl']);
				return $r;
			}
			if($method == 'https')
			{
					...
					((is_string($info) && strpos($info, 'OpenSSL') === null) || (is_array($info) && !isset($info['ssl_version']))))
				{
					...
					$r=new Response(0, $GLOBALS['xmlrpcerr']['no_ssl'], $GLOBALS['xmlrpcstr']['no_ssl']);
					return $r;
				}
			}
			...
			if(!$result)
			{
				...
				$resp=new Response(0, $GLOBALS['xmlrpcerr']['curl_fail'], $GLOBALS['xmlrpcstr']['curl_fail']. ': '. curl_error($curl));
				curl_close($curl);
				if($keepalive)
				{
		...
		function multicall($msgs, $timeout=10, $method='', $fallback=true)
		{
			...
			if(!$this->no_multicall)
			{
				...
				else
				{
					...
					{
						if (is_a($results, '\\JExtstore\\Component\\JMap\\Administrator\\Framework\\Pinger\\Xmlrpc\\Response'))
						{
							$result = $results;
						}
						else
						{
							$result = new Response(0, $GLOBALS['xmlrpcerr']['multicall_error'], $GLOBALS['xmlrpcstr']['multicall_error']);
						}
					}
				}
			}
		...
		function _try_multicall($msgs, $timeout, $method)
		{
			...
			$multicall = new Message('system.multicall');
...
} // end class \JExtstore\Component\JMap\Administrator\Framework\Pinger\Xmlrpc\Client
	...
	function __construct($val, $fcode = 0, $fstr = '', $valtyp='')
	{
		...
		else
		{
			...
			if ($valtyp == '')
			{
				...
				if (is_object($this->val) && is_a($this->val, '\\JExtstore\\Component\\JMap\\Administrator\\Framework\\Pinger\\Xmlrpc\\Value'))
				{
...
"</struct>\n</value>\n</fault>";
		...
		else
		{
			if(!is_object($this->val) || !is_a($this->val, '\\JExtstore\\Component\\JMap\\Administrator\\Framework\\Pinger\\Xmlrpc\\Value'))
			{
				...
				{
					die('cannot serialize Response objects whose content is native php values');
				}
			}
			else
			{
...
class Message
{
	...
	function addParam($par)
	{
		...
		if(is_object($par) && is_a($par, '\\JExtstore\\Component\\JMap\\Administrator\\Framework\\Pinger\\Xmlrpc\\Value'))
		{
			$this->params[]=$par;
			return true;
		}
	...
	function &parseResponseHeaders(&$data, $headers_processed=false)
	{
			...
			if(preg_match('/^HTTP\/1\.[0-1] 200 Connection established/', $data))
			{
				...
				{
					$r=new Response(0, $GLOBALS['xmlrpcerr']['http_error'], $GLOBALS['xmlrpcstr']['http_error']. ' (HTTPS via proxy error, tunnel connection possibly failed)');
					return $r;
				}
			}
			...
			if(!preg_match('/^HTTP\/[0-9.]+ 200 /', $data))
			{
				...
				$r=new Response(0, $GLOBALS['xmlrpcerr']['http_error'], $GLOBALS['xmlrpcstr']['http_error']. ' (' . $errstr . ')');
				return $r;
			}
			...
			if(!$headers_processed)
			{
				...
				if(isset($GLOBALS['_xh']['headers']['transfer-encoding']) && $GLOBALS['_xh']['headers']['transfer-encoding'] == 'chunked')
				{
					...
					{
						$r = new Response(0, $GLOBALS['xmlrpcerr']['dechunk_fail'], $GLOBALS['xmlrpcstr']['dechunk_fail']);
						return $r;
					}
				}
				...
				if(isset($GLOBALS['_xh']['headers']['content-encoding']))
				{
					...
					if($GLOBALS['_xh']['headers']['content-encoding'] == 'deflate' || $GLOBALS['_xh']['headers']['content-encoding'] == 'gzip')
					{
						...
						if(function_exists('gzinflate'))
						{
							...
							{
								$r = new Response(0, $GLOBALS['xmlrpcerr']['decompress_fail'], $GLOBALS['xmlrpcstr']['decompress_fail']);
								return $r;
							}
						}
						else
						{
							$r = new Response(0, $GLOBALS['xmlrpcerr']['cannot_decompress'], $GLOBALS['xmlrpcstr']['cannot_decompress']);
							return $r;
						}
					}
				}
	...
	function &parseResponse($data='', $headers_processed=false, $return_type='xmlrpcvals')
	{
		...
		{
			xml_set_element_handler($parser, '\JExtstore\Component\JMap\Administrator\Framework\Pinger\Xmlrpc\jmap_xmlrpc_se', '\JExtstore\Component\JMap\Administrator\Framework\Pinger\Xmlrpc\jmap_xmlrpc_ee_fast');
		}
		else
		{
			xml_set_element_handler($parser, '\JExtstore\Component\JMap\Administrator\Framework\Pinger\Xmlrpc\jmap_xmlrpc_se', '\JExtstore\Component\JMap\Administrator\Framework\Pinger\Xmlrpc\jmap_xmlrpc_ee');
		}

		xml_set_character_data_handler($parser, '\JExtstore\Component\JMap\Administrator\Framework\Pinger\Xmlrpc\jmap_xmlrpc_cd');
		xml_set_default_handler($parser, '\JExtstore\Component\JMap\Administrator\Framework\Pinger\Xmlrpc\jmap_xmlrpc_dh');
		...
		if(!xml_parse($parser, $data, $isDataTrue))
		{
			...
			$r=new Response(0, $GLOBALS['xmlrpcerr']['invalid_return'], $GLOBALS['xmlrpcstr']['invalid_return'].' ('.$errstr.')');
...
class Value {
	...
	{
		reset($this->me);
		list($a,$b)=jmap_xmlrpc_each($this->me);
		if(is_array($b))
		{
			@reset($b);
			while(list($id,$cont) = @jmap_xmlrpc_each($b))
			{
				$b[$id] = $cont->scalarval();
			}
		}
		...
		if(is_object($b))
		{
			$t = get_object_vars($b);
			@reset($t);
			while(list($id,$cont) = @jmap_xmlrpc_each($t))
			{
				$t[$id] = $cont->scalarval();
			}
			@reset($t);
	...
	{
		reset($this->me);
		list(,$b)=jmap_xmlrpc_each($this->me);
		return $b;
	}
	...
	{
		reset($this->me);
		list($a,)=jmap_xmlrpc_each($this->me);
	...
	switch($type)
	{
		case 'string':
			if (in_array('auto_dates', $options) && preg_match('/^[0-9]{8}T[0-9]{2}:[0-9]{2}:[0-9]{2}$/', $php_val))
				$xmlrpc_val = new Value($php_val, $GLOBALS['xmlrpcDateTime']);
			else
				$xmlrpc_val = new Value($php_val, $GLOBALS['xmlrpcString']);
			break;
		case 'integer':
			$xmlrpc_val = new Value($php_val, $GLOBALS['xmlrpcInt']);
			break;
		case 'double':
			$xmlrpc_val = new Value($php_val, $GLOBALS['xmlrpcDouble']);
			break;
			// Add support for encoding/decoding of booleans, since they are supported in PHP
		case 'boolean':
			$xmlrpc_val = new Value($php_val, $GLOBALS['xmlrpcBoolean']);
			break;
		case 'array':
			$j = 0;
			$arr = array();
			...
			if($ko)
			{
				$xmlrpc_val = new Value($arr, $GLOBALS['xmlrpcStruct']);
			}
			else
			{
				$xmlrpc_val = new Value($arr, $GLOBALS['xmlrpcArray']);
			}
			break;
		case 'object':
			if(is_a($php_val, '\\JExtstore\\Component\\JMap\\Administrator\\Framework\\Pinger\\Xmlrpc\\Value'))
			{
				$xmlrpc_val = $php_val;
			}
			else
			{
				...
				{
					// let's save original class name into Value:
		...
		case 'NULL':
			if (in_array('extension_api', $options))
			{
				$xmlrpc_val = new Value('', $GLOBALS['xmlrpcString']);
			}
			if (in_array('null_extension', $options))
			{
				$xmlrpc_val = new Value('', $GLOBALS['xmlrpcNull']);
			}
			else
			{
		...
		case 'resource':
			if (in_array('extension_api', $options))
			{
				$xmlrpc_val = new Value((int)$php_val, $GLOBALS['xmlrpcInt']);
			}
			else
			{
				$xmlrpc_val = new Value();
	...
	if ($GLOBALS['_xh']['isf'] < 2)
	{
		...
		switch($name)
		{
			case 'VALUE':
				...
				{
					$temp = new Value($GLOBALS['_xh']['value'], $GLOBALS['_xh']['vt']);
