(function (document, Joomla) {
    ...
    async function verifyBuyer(payments, token) {
        ...
        if (form.payment_type !== undefined && form.payment_type.value === '1') {
            verificationDetails.amount = getFormInputValue(form.querySelector('#deposit_amount'));
        } else {
            verificationDetails.amount = getFormInputValue(form.querySelector('#amount'));

            if (verificationDetails.amount === '') {
                verificationDetails.amount = getFormInputValue(form.querySelector('#total_amount'));
            }
        }
