...
class book_dates_detailController extends JControllerForm
{
	...
	function __construct( $default = array())
	{
		...
		$this->registerTask( 'create_book_date_series', 'create_book_date_series' );
	...
	*/
	function edit($key=null, $urlVar=null)
	{
		...
		// Checkin the book_dates
		$model = $this->getModel('book_dates_detail');
		$model->checkout();
	}
	...
	*/
	function save($key=null, $urlVar=null)
	{
		...
		$model->checkin();
		if($key=="2new"){
			$this->setRedirect( 'index.php?option=com_rsappt_pro3&controller=book_dates_detail&task=edit&cid[]=-1',$msg );
		} else {
			$this->setRedirect( 'index.php?option=com_rsappt_pro3&controller=book_dates',$msg );
		}
	...
	*/
	...
	/** function saveorder
	*
	* saveorder of the book_dates items
	* 			
	* @param array cid		- array of id
	...
	*/
	...
	function create_book_date_series(){
		$jinput = JFactory::getApplication()->input;
		...
		$rolling_book_date = $jinput->getString('rolling_book_date');
		$book_date_starttime = $jinput->getString('book_date_starttime');
		$book_date_endtime = $jinput->getString('book_date_endtime');
		...
		while($d1 <= $d2){
			...
			if(strpos($bo_days, $process_wday) >0 ) {
				$sql = "INSERT INTO #__sv_apptpro3_book_dates (resource_id,description,off_date,full_day,book_date_starttime,book_date_endtime,rolling_book_date,published)".
				" VALUES(".$resource.",'".$database->escape($resource_desc)."','".date("Y-m-d", $d1)."','".$full_day."','".$book_date_starttime."','".$book_date_endtime."','".$rolling_book_date."',".$published.")";
				try{
					$database->setQuery( $sql );
					$database->execute();
				} catch (RuntimeException $e) {
					logIt($e->getMessage(), "be_ctrl_book_dates_detail", "", "");
