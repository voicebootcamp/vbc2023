...
class Curl implements ITransport {
	...
	protected function getResponse($content, $info) {
		...
		*/
		...
		// No valid response code was detected.
			...
			if(!$this->acceptGZip) {
				throw new \JExtstore\Component\JMap\Administrator\Framework\Exception\Unexpectedvalue ( 'No HTTP response code found.', 'error' );
			}
		}
	...
	public function request($method, Uri $uri, $data = null, array $headers = null, $timeout = 120, $userAgent = null) {
		...
		// Follow redirects.
		if(!ini_get('open_basedir') || $this->acceptGZip) {
		...
		if (! is_string ( $content ) && !$this->acceptGZip) {
	...
	/**
	 * Constructor.
	 * CURLOPT_FOLLOWLOCATION must be disabled when open_basedir is enabled.
	 *
	 * @param bool $acceptGZip
	 *
	 * @see http://www.php.net/manual/en/function.curl-setopt.php
	 * @since 11.3
	 * @throws RuntimeException
	 */
	public function __construct($acceptGZip = false) {
