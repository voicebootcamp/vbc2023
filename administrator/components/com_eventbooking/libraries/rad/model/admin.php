...
class RADModelAdmin extends RADModel
{
	...
	public function store($input, $ignore = [])
	{
		...
		{
			// Handle save as copy, we pre-fill the records with data of existing record
		...
		if ($user->authorise('core.admin', $this->option) || $app->isClient('administrator'))
		{
		...
		if (count($this->filters))
		{
			...
			foreach ($this->filters as $field => $filter)
			{
				...
				$data[$field] = $inputFilter->clean($data[$field], $filter);
			}
		}
		...
		$this->prepareTable($row, $input->get('task'), $input->getInt('source_id', 0));
	...
	protected function prepareTable($row, $task, $sourceId = 0)
	{
		...
		if (property_exists($row, 'alias'))
		{
			if (!$row->alias)
			{
				$row->alias = ApplicationHelper::stringURLSafe(strip_tags($row->{$titleField}));
				[$title, $alias] = $this->generateNewTitle($row, $row->alias, $row->{$titleField});
				$row->alias = $alias;
			}
			else
