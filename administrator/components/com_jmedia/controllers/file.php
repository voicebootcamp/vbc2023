...
/**
 * JMedia File Controller
 *
 * @since  1.5
 */
class JMediaControllerFile extends JControllerLegacy
{
	/**
	...
	public function upload()
	{
		...
		$return       = JFactory::getSession()->get('com_jmedia.return_url');
		...
		{
			$this->setRedirect('index.php?option=com_jmedia&folder=' . $this->folder);
		}

		if (!$files)
		{
			// If we could not get any data from the request we can not upload it.
			JFactory::getApplication()->enqueueMessage(JText::_('COM_JMEDIA_ERROR_WARNFILENOTSAFE'), 'error');
		...
		{
			JError::raiseWarning(100, JText::_('COM_JMEDIA_ERROR_WARNUPLOADTOOLARGE'));
		...
		foreach ($files as &$file)
		{
			...
			$fileparts = pathinfo(COM_JMEDIA_BASE . '/' . $this->folder . '/' . $file['name']);

			if (strpos(realpath($fileparts['dirname']), JPath::clean(COM_JMEDIA_BASE)) !== 0)
			{
			...
			$file['filepath'] = JPath::clean(implode(DIRECTORY_SEPARATOR, array(COM_JMEDIA_BASE, $this->folder, $file['name'])));
				...
				|| ($uploadMaxFileSize > 0 && $file['size'] > $uploadMaxFileSize))
			{
				...
				JError::raiseWarning(100, JText::_('COM_JMEDIA_ERROR_WARNFILETOOLARGE'));
			...
			if (JFile::exists($file['filepath']))
			{
				...
				JError::raiseWarning(100, JText::_('COM_JMEDIA_ERROR_FILE_EXISTS'));
			...
			if (!isset($file['name']))
			{
				...
				$this->setRedirect('index.php', JText::_('COM_JMEDIA_INVALID_REQUEST'), 'error');
		...
		foreach ($files as &$file)
		{
			...
			$result = $dispatcher->trigger('onContentBeforeSave', array('com_jmedia.file', &$object_file, true));
			...
			{
				// Error in upload
				JError::raiseWarning(100, JText::_('COM_JMEDIA_ERROR_UNABLE_TO_UPLOAD_FILE'));

				return false;
			}

			// Trigger the onContentAfterSave event.
			$dispatcher->trigger('onContentAfterSave', array('com_jmedia.file', &$object_file, true));
			$this->setMessage(JText::sprintf('COM_JMEDIA_UPLOAD_COMPLETE', substr($object_file->filepath, strlen(COM_JMEDIA_BASE))));
		}
	...
	{
		if (!JFactory::getUser()->authorise('core.' . strtolower($action), 'com_jmedia'))
		{
	...
	public function delete()
	{
		...
		{
			$fullPath = implode(DIRECTORY_SEPARATOR, array(COM_JMEDIA_BASE, $folder, $path));
