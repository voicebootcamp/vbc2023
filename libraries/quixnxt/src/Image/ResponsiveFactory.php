...
use QuixNxt\Image\Config\ResponsiveFactoryConfigurator;
use QuixNxt\Image\Exception\FileNotFoundException;
...
class ResponsiveFactory
{
    ...
    public function __construct(ResponsiveFactoryConfigurator $configurator = null)
    {
        ...
        if (!$this->fs->exists($this->publicPath)) {
            try {
                ...
                if (!$this->fs->exists($this->publicPath)) {
                    echo new JResponseJSON($e); jexit();
                }
            }
    ...
    public function createScaledImages(SplFileInfo $sourceImage, ResponsiveImage $responsiveImage) : ResponsiveImage
    {
        ...
        [$sizes, $responsiveBreakPoints] = $this->scaler->scale($sourceImage, $imageObject);

        foreach ($sizes as $width => $height) {
            $breakPoint = $responsiveBreakPoints[$width];

            $scaledFileSrc = trim("{$urlPath}/{$imageObject->filename}-{$breakPoint}.{$imageObject->extension}", '/');
