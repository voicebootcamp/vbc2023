...
class EventbookingModelCommonEvent extends RADModelAdmin
{
	...
	public function store($input, $ignore = [])
	{
		...
		elseif ($input->getInt('source_id'))
		{
			...
			$input->set('created_by', Factory::getUser()->id);
			$row->id   = 0;
			$row->hits = 0;
		}
		else
	...
	protected function storeRecurringEvent(&$data, $sourceId, $input = null, $row = null)
	{
		...
		{
			[$eventDatesDate, $deleteEventIds] = $this->updateChildrenEvents($row, $eventDates, $eventDuration, $cutOffDuration, $cancelDuration, $earlyBirdDuration, $registrationStartDuration);
	...
	protected function addNewChildrenEvents($row, $eventDates, $task, $eventDuration, $cutOffDuration, $cancelDuration, $earlyBirdDuration, $registrationStartDuration)
	{
		...
		if ((int) $row->publish_up)
		{
			$publishUpDuration = strtotime($row->publish_up) - strtotime($row->event_date);
		}
		else
		{
			$publishUpDuration = false;
		}

		if ((int) $row->publish_down)
		{
			$publishDownDuration = strtotime($row->publish_down) - strtotime($row->event_date);
		}
		else
		{
			$publishDownDuration = false;
		}
		...
		foreach ($eventDates as $eventDate)
		{
			...
			if ($publishUpDuration !== false)
			{
				$rowChildEvent->publish_up = strftime('%Y-%m-%d %H:%M:%S', strtotime($rowChildEvent->event_date) + $publishUpDuration);
			}

			if ($publishDownDuration !== false)
			{
				$rowChildEvent->publish_down = strftime('%Y-%m-%d %H:%M:%S', strtotime($rowChildEvent->event_date) + $publishDownDuration);
			}
			...
			$rowChildEvent->alias                  = strip_tags($rowChildEvent->title) . '-' . HTMLHelper::_('date', $rowChildEvent->event_date, $config->date_format, null);
			...
			if ($isMultilingual)
			{
				...
				{
					$sef                              = $language->sef;
					$rowChildEvent->{'alias_' . $sef} = ApplicationHelper::stringURLSafe(strip_tags($rowChildEvent->{'title_' . $sef}) . '-' . HTMLHelper::_('date', $rowChildEvent->event_date, $config->date_format, null));
				}
			}
	...
	protected function updateChildrenEvents($row, $eventDates, $eventDuration, $cutOffDuration, $cancelDuration, $earlyBirdDuration, $registrationStartDuration)
	{
		...
		if (count($children))
		{
			...
			foreach ($children as $childId)
			{
				...
				if ($cancelDuration || (int) $row->cancel_before_date)
				{
					$rowChildEvent->cancel_before_date = strftime('%Y-%m-%d %H:%M:%S', strtotime($rowChildEvent->event_date) - $cancelDuration);
				}
				else
	...
	public function exportRegistrantsOfEventToFile($eventId, $filename = 'registrants_list.xlsx', $excludeStatuses = [])
	{
		...
		if (count($rows))
		{
			...
			[$fields, $headers] = EventbookingHelper::callOverridableHelperMethod('Data', 'prepareRegistrantsExportData', [$rows, $config, $rowFields, $fieldValues, $eventId]);
