...
class SourcesModel extends JMapModel implements IModelSources {
	...
	protected function buildListQuery() {
		...
		if ($language && $language != '*' && $this->getLanguagePluginEnabled()) {
			$where [] = "s.params LIKE " . $this->dbInstance->quote('%"datasource_language":"' . $this->state->get ( 'language' ) . '"%');
		}
	...
	public function getLists($record = null): array {
		...
		array_unshift($elements, HTMLHelper::_('select.option', null, Text::_('COM_JMAP_SELECTCOMPONENT')));
		...
		$operatorOptions[] = HTMLHelper::_('select.option', null, '=');
		...
		$directionOptions[] = HTMLHelper::_('select.option', null, Text::_('COM_JMAP_SELECTFIELD'));
		...
		$joinOptions = array();
		$joinOptions[] = HTMLHelper::_('select.option', null, Text::_('COM_JMAP_DEFAULT_JOIN'));
		...
		$options = array();
		$options[] = HTMLHelper::_('select.option', null, Text::_('COM_JMAP_SELECTPRIORITY')); 
		...
		$lists ['priorities'] = HTMLHelper::_ ( 'select.genericlist', $options, 'params[priorities]', 'class="custom-form-select" style="width:200px" size="15"', 'value', 'text', null, 'priorities');
		
		// Change frequency select list
		$options = array();
		$options[] = HTMLHelper::_('select.option', null, Text::_('COM_JMAP_SELECTCHANGEFREQ'));
		...
		if($record->type == 'user') {
			...
			$lists['sef_itemid'] = HTMLHelper::_('select.genericlist', $sefItemid, 'params[sef_itemid]', 'size="15"', 'value', 'text', $record->params->get('sef_itemid', null), 'sef_itemid' );
		}
