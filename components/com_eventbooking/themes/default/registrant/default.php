	...
	<form action="<?php echo Route::_('index.php?option=com_eventbooking&view=registrants&Itemid=' . $this->Itemid); ?>" method="post" name="adminForm" id="adminForm" class="<?php echo $formHorizontalClass; ?>">
		...
		<div class="<?php echo $controlGroupClass; ?>">
			...
			<div class="<?php echo $controlsClass; ?>">
				...
				{
				?>
					<input class="form-control input-small validate[required,custom[number]]" type="text" name="number_registrants"
		...
		if ($this->item->deposit_amount > 0 || ($this->config->activate_deposit_feature && $this->canChangeStatus))
		{
		?>
			<div class="<?php echo $controlGroupClass; ?>">
				<div class="<?php echo $controlLabelClass; ?>">
				...
				<div class="<?php echo $controlsClass; ?>">
					...
					if ($this->canChangeStatus)
					{
						$input = '<input type="text" class="input-medium form-control" name="deposit_amount" value="' . round($this->item->deposit_amount,
								$numberDecimals) . '" size="7" />';
						echo $bootstrapHelper->getPrependAddon($input, $this->config->currency_symbol);
					}
					else
					{
						echo EventbookingHelper::formatCurrency($this->item->deposit_amount, $this->config);
					}
					?>
				</div>
			</div>
