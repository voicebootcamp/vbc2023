...
class HTML_OSappscheduleWorktime{
	 ...
	 */
	static function worktime_list($option,$rows){
	...
	?>
		<form method="POST" action="index.php?option=<?php echo $option; ?>&task=worktime_list" name="adminForm" id="adminForm">
			<table id="workingListTable" width="100%" class="adminlist table table-striped">
				<thead>
					<tr>
						<th width="5%">#</th>
						<th>
					...
					<tr class="<?php echo "row$k";?>">
						...
						<td align="center" >
							<?php  
								...
								echo HelperDateTime::CreatDropHour('start_time_hour_'.$row->id,(int)$row->start_time_hour,'class="'.$mapClass['input-mini'].' form-select is_day_'. $row->id.'"');
								echo HelperDateTime::CreatDropMinuste('start_time_minutes_'.$row->id,(int)$row->start_time_minutes,'class="'.$mapClass['input-mini'].' form-select is_day_'. $row->id.'"')
							?>
						</td>
						<td align="center">
							<?php 
								list($row->end_time_hour,$row->end_time_minutes) 	= explode(':',$row->end_time); 
								echo HelperDateTime::CreatDropHour('end_time_hour_'.$row->id,(int)$row->end_time_hour,'class="'.$mapClass['input-mini'].' form-select is_day_'. $row->id.'"');
								echo HelperDateTime::CreatDropMinuste('end_time_minutes_'.$row->id,(int)$row->end_time_minutes,'class="'.$mapClass['input-mini'].' form-select is_day_'. $row->id.'"')
							?>
						</td>
