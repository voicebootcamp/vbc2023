...
?>
	<<?php echo $hTag; ?> class="eb-page-heading"><?php echo $this->params->get('page_heading') ?: $this->escape(Text::_('EB_USER_EVENTS')); ?></<?php echo $hTag; ?>>
<?php
}
...
{
?>
	<div class="eb-description eb-events-management-intro-text"><?php echo $this->params->get('intro_text');?></div>
<?php
}
?>
		...
		?>
			<table class="<?php echo $bootstrapHelper->getClassMapping('table table-striped table-bordered table-considered'); ?>">
				...
				<tbody>
					...
					{
						$row      = $this->items[$i];
						$link     = Route::_(EventbookingHelperRoute::getEventRoute($row->id, $row->main_category_id, $Itemid) . '&layout=default');
