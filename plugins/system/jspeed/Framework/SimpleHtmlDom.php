...
define('JSPEED_HDOM_TYPE_ELEMENT', 1);
define('JSPEED_HDOM_TYPE_COMMENT', 2);
define('JSPEED_HDOM_TYPE_TEXT', 3);
define('JSPEED_HDOM_TYPE_ENDTAG', 4);
define('JSPEED_HDOM_TYPE_ROOT', 5);
define('JSPEED_HDOM_TYPE_UNKNOWN', 6);
define('JSPEED_HDOM_QUOTE_DOUBLE', 0);
define('JSPEED_HDOM_QUOTE_SINGLE', 1);
define('JSPEED_HDOM_QUOTE_NO', 3);
define('JSPEED_HDOM_INFO_BEGIN', 0);
define('JSPEED_HDOM_INFO_END', 1);
define('JSPEED_HDOM_INFO_QUOTE', 2);
define('JSPEED_HDOM_INFO_SPACE', 3);
define('JSPEED_HDOM_INFO_TEXT', 4);
define('JSPEED_HDOM_INFO_INNER', 5);
define('JSPEED_HDOM_INFO_OUTER', 6);
define('JSPEED_HDOM_INFO_ENDSPACE', 7);

/** The default target charset */
defined('JSPEED_DEFAULT_TARGET_CHARSET') || define('JSPEED_DEFAULT_TARGET_CHARSET', 'UTF-8');

/** The default <br> text used instead of <br> tags when returning text */
defined('JSPEED_DEFAULT_BR_TEXT') || define('JSPEED_DEFAULT_BR_TEXT', "\r\n");

/** The default <span> text used instead of <span> tags when returning text */
defined('JSPEED_DEFAULT_SPAN_TEXT') || define('JSPEED_DEFAULT_SPAN_TEXT', ' ');

/** The maximum file size the parser should load */
defined('JSPEED_MAX_FILE_SIZE') || define('JSPEED_MAX_FILE_SIZE', 600000);

/** Contents between curly braces "{" and "}" are interpreted as text */
define('JSPEED_HDOM_SMARTY_AS_TEXT', 1);

// helper functions
// -----------------------------------------------------------------------------
// get html dom from file
// $maxlen is defined in the code as PHP_STREAM_COPY_ALL which is defined as -1.
function simplehtmldom_file_get_html(
	$url,
	...
	$target_charset = JSPEED_DEFAULT_TARGET_CHARSET,
	$stripRN = true,
	$defaultBRText = JSPEED_DEFAULT_BR_TEXT,
	$defaultSpanText = JSPEED_DEFAULT_SPAN_TEXT)
{
	// Ensure maximum length is greater than zero
	if($maxLen <= 0) { $maxLen = JSPEED_MAX_FILE_SIZE; }
...
function simplehtmldom_str_get_html(
	$str,
	$lowercase = true,
	$forceTagsClosed = true,
	$target_charset = JSPEED_DEFAULT_TARGET_CHARSET,
	...
	$defaultSpanText = JSPEED_DEFAULT_SPAN_TEXT)
{
	...
	if (empty($str) || strlen($str) > JSPEED_MAX_FILE_SIZE) {
		$dom->clear();
		return false;
	}
...
class SimpleHtmlDomNode
{
	 ...
	 * Node type
	 *
	 * Default is {@see JSPEED_HDOM_TYPE_TEXT}
	 *
	 * @var int
	 */
	public $nodetype = JSPEED_HDOM_TYPE_TEXT;
	...
	// The "info" array - see JSPEED_HDOM_INFO_... for what each element contains.
	public $_ = array();
	...
	function dump_node($echo = true)
	{
		...
		$string .= " JSPEED_HDOM_INNER_INFO: '";

		if (isset($node->_[JSPEED_HDOM_INFO_INNER])) {
			$string .= $node->_[JSPEED_HDOM_INFO_INNER] . "'";
		} else {
			$string .= ' NULL ';
		}
	...
	function innertext()
	{
		if (isset($this->_[JSPEED_HDOM_INFO_INNER])) {
			return $this->_[JSPEED_HDOM_INFO_INNER];
		}

		if (isset($this->_[JSPEED_HDOM_INFO_TEXT])) {
			return $this->dom->restore_noise($this->_[JSPEED_HDOM_INFO_TEXT]);
		}
	...
	function outertext()
	{
		...
		if (isset($this->_[JSPEED_HDOM_INFO_OUTER])) {
			return $this->_[JSPEED_HDOM_INFO_OUTER];
		}
		...
		// render begin tag
		if ($this->dom && $this->dom->nodes[$this->_[JSPEED_HDOM_INFO_BEGIN]]) {
			$ret = $this->dom->nodes[$this->_[JSPEED_HDOM_INFO_BEGIN]]->makeup();
		} else {
			$ret = '';
		}
		...
		// render inner text
		if (isset($this->_[JSPEED_HDOM_INFO_INNER])) {
			// If it's a br tag...  don't return the JSPEED_HDOM_INNER_INFO that we
			// may or may not have added.
			if ($this->tag !== 'br') {
				$ret .= $this->_[JSPEED_HDOM_INFO_INNER];
			}
		} else {
			if ($this->nodes) {
				foreach ($this->nodes as $n) {
		...
		// render end tag
		if (isset($this->_[JSPEED_HDOM_INFO_END]) && $this->_[JSPEED_HDOM_INFO_END] != 0) {
			$ret .= '</' . $this->tag . '>';
		}
	...
	function text()
	{
		...
		switch ($this->nodetype) {
			case JSPEED_HDOM_TYPE_TEXT: return $this->dom->restore_noise($this->_[JSPEED_HDOM_INFO_TEXT]);
			case JSPEED_HDOM_TYPE_COMMENT: return '';
			case JSPEED_HDOM_TYPE_UNKNOWN: return '';
		}
		...
		// In rare cases, (always node type 1 or JSPEED_HDOM_TYPE_ELEMENT - observed
	...
	function makeup()
	{
		...
		foreach ($this->attr as $key => $val) {
			...
			$ret .= $this->_[JSPEED_HDOM_INFO_SPACE][$i][0];

			//no value attr: nowrap, checked selected...
			if ($val === true) {
				$ret .= $key;
			} else {
				switch ($this->_[JSPEED_HDOM_INFO_QUOTE][$i])
				{
					case JSPEED_HDOM_QUOTE_DOUBLE: $quote = '"'; break;
					case JSPEED_HDOM_QUOTE_SINGLE: $quote = '\''; break;
					default: $quote = '';
				}

				$ret .= $key
				. $this->_[JSPEED_HDOM_INFO_SPACE][$i][1]
				. '='
				. $this->_[JSPEED_HDOM_INFO_SPACE][$i][2]
				. $quote
				. $val
				. $quote;
			}
		...
		return $ret . $this->_[JSPEED_HDOM_INFO_ENDSPACE] . '>';
	}
	...
	function find($selector, $idx = null, $lowercase = false)
	{
		...
		for ($c = 0; $c < $count; ++$c) {
			...
			if (!isset($this->_[JSPEED_HDOM_INFO_BEGIN])) { return array(); }

			$head = array($this->_[JSPEED_HDOM_INFO_BEGIN] => 1);
			$cmd = ' '; // Combinator
	...
	protected function seek($selector, &$ret, $parent_cmd, $lowercase = false)
	{
		...
		if ($parent_cmd === ' ') { // Descendant Combinator
			...
			$end = (!empty($this->_[JSPEED_HDOM_INFO_END])) ? $this->_[JSPEED_HDOM_INFO_END] : 0;
			if ($end == 0) {
				$parent = $this->parent;
				while (!isset($parent->_[JSPEED_HDOM_INFO_END]) && $parent !== null) {
					$end -= 1;
					$parent = $parent->parent;
				}
				$end += $parent->_[JSPEED_HDOM_INFO_END];
			}

			// Get list of target nodes
			$nodes_start = $this->_[JSPEED_HDOM_INFO_BEGIN] + 1;
		...
		foreach($nodes as $node) {
			...
			if ($pass) $ret[$node->_[JSPEED_HDOM_INFO_BEGIN]] = 1;
			unset($node);
		}
	...
	function __set($name, $value)
	{
		...
		switch ($name) {
			case 'outertext': return $this->_[JSPEED_HDOM_INFO_OUTER] = $value;
			case 'innertext':
				if (isset($this->_[JSPEED_HDOM_INFO_TEXT])) {
					return $this->_[JSPEED_HDOM_INFO_TEXT] = $value;
				}
				return $this->_[JSPEED_HDOM_INFO_INNER] = $value;
		}

		if (!isset($this->attr[$name])) {
			$this->_[JSPEED_HDOM_INFO_SPACE][] = array(' ', '', '');
			$this->_[JSPEED_HDOM_INFO_QUOTE][] = JSPEED_HDOM_QUOTE_DOUBLE;
		}
...
class SimpleHtmlDom
{
	 ...
	 * string length of ({@see SimpleHtmlDom::$doc}).
	 *
	 ...
	 * Current character
	 *
	 * Holds the current character at position {@see SimpleHtmlDom::$pos} in
	 * the document {@see SimpleHtmlDom::$doc}
	 *
		...
		$options = 0)
	{
		...
		if($options & JSPEED_HDOM_SMARTY_AS_TEXT) { // Strip Smarty scripts
		...
		$this->parse();
		// end
		$this->root->_[JSPEED_HDOM_INFO_END] = $this->cursor;
		$this->parse_charset();
		...
		$defaultSpanText = JSPEED_DEFAULT_SPAN_TEXT)
	{
		...
		$this->root->tag = 'root';
		$this->root->_[JSPEED_HDOM_INFO_BEGIN] = -1;
		$this->root->nodetype = JSPEED_HDOM_TYPE_ROOT;
	...
	protected function parse()
	{
		while (true) {
			...
			++$this->cursor;
			$node->_[JSPEED_HDOM_INFO_TEXT] = $s;
	...
	protected function read_tag()
	{
		...
		if ($this->char === '/') {
			...
			if ($parent_lower !== $tag_lower) {
				...
				if (isset($this->optional_closing_tags[$parent_lower])
					...
					$this->parent->_[JSPEED_HDOM_INFO_END] = 0;
					...
					if (strtolower($this->parent->tag) !== $tag_lower) {
						...
						$this->parent->_[JSPEED_HDOM_INFO_END] = $this->cursor;
					...
					$this->parent->_[JSPEED_HDOM_INFO_END] = 0; // No end tag
					$org_parent = $this->parent;
		...
		// start tag
		$node = new SimpleHtmlDomNode($this);
		$node->_[JSPEED_HDOM_INFO_BEGIN] = $this->cursor;
		++$this->cursor;
		...
		if (isset($tag[0]) && $tag[0] === '!') {
			$node->_[JSPEED_HDOM_INFO_TEXT] = '<' . $tag . $this->copy_until_char('>');

			if (isset($tag[2]) && $tag[1] === '-' && $tag[2] === '-') { // Comment ("<!--")
				$node->nodetype = JSPEED_HDOM_TYPE_COMMENT;
				$node->tag = 'comment';
			} else { // Could be doctype or CDATA but we don't care
				$node->nodetype = JSPEED_HDOM_TYPE_UNKNOWN;
				$node->tag = 'unknown';
			}

			if ($this->char === '>') { $node->_[JSPEED_HDOM_INFO_TEXT] .= '>'; }
		...
		if ($pos = strpos($tag, '<') !== false) {
			$tag = '<' . substr($tag, 0, -1);
			$node->_[JSPEED_HDOM_INFO_TEXT] = $tag;
			$this->link_nodes($node, false);
		...
		if (!preg_match('/^\w[\w:-]*$/', $tag)) {
			$node->_[JSPEED_HDOM_INFO_TEXT] = '<' . $tag . $this->copy_until('<>');
		...
		$node->nodetype = JSPEED_HDOM_TYPE_ELEMENT;
		$tag_lower = strtolower($tag);
		...
		do {
			...
			if ($this->pos >= $this->size - 1 && $this->char !== '>') {
				$node->nodetype = JSPEED_HDOM_TYPE_TEXT;
				$node->_[JSPEED_HDOM_INFO_END] = 0;
				$node->_[JSPEED_HDOM_INFO_TEXT] = '<' . $tag . $space[0] . $name;
				$node->tag = 'text';
				$this->link_nodes($node, false);
				return true;
			}
			...
			if ($this->doc[$this->pos - 1] == '<') {
				...
				$node->attr = array();
				$node->_[JSPEED_HDOM_INFO_END] = 0;
				$node->_[JSPEED_HDOM_INFO_TEXT] = substr(
					$this->doc,
					$begin_tag_pos,
			...
			if ($name !== '/' && $name !== '') { // this is a attribute name
				...
				if ($this->char === '=') { // attribute with value
					...
					//no value attr: nowrap, checked selected...
					$node->_[JSPEED_HDOM_INFO_QUOTE][] = JSPEED_HDOM_QUOTE_NO;
					$node->attr[$name] = true;
					if ($this->char != '>') { $this->char = $this->doc[--$this->pos]; } // prev
				}

				$node->_[JSPEED_HDOM_INFO_SPACE][] = $space;
		...
		$this->link_nodes($node, true);
		$node->_[JSPEED_HDOM_INFO_ENDSPACE] = $space[0];

		// handle empty tags (i.e. "<div/>")
		if ($this->copy_until_char('>') === '/') {
			$node->_[JSPEED_HDOM_INFO_ENDSPACE] .= '/';
			$node->_[JSPEED_HDOM_INFO_END] = 0;
		} else {
			// reset parent
		...
		if ($node->tag === 'br') {
			$node->_[JSPEED_HDOM_INFO_INNER] = $this->default_br_text;
		}
	...
	protected function parse_attr($node, $name, &$space)
	{
		...
		switch ($this->char) {
			case '"': // value is anything between double quotes
				$node->_[JSPEED_HDOM_INFO_QUOTE][] = JSPEED_HDOM_QUOTE_DOUBLE;
				$this->char = (++$this->pos < $this->size) ? $this->doc[$this->pos] : null; // next
				$node->attr[$name] = $this->restore_noise($this->copy_until_char('"'));
				$this->char = (++$this->pos < $this->size) ? $this->doc[$this->pos] : null; // next
				break;
			case '\'': // value is anything between single quotes
				$node->_[JSPEED_HDOM_INFO_QUOTE][] = JSPEED_HDOM_QUOTE_SINGLE;
				$this->char = (++$this->pos < $this->size) ? $this->doc[$this->pos] : null; // next
				$node->attr[$name] = $this->restore_noise($this->copy_until_char('\''));
				$this->char = (++$this->pos < $this->size) ? $this->doc[$this->pos] : null; // next
				break;
	 ...
	 *
	 * Noise is stored to {@see SimpleHtmlDom::$noise}
	 *
	 ...
	 *
	 * Noise is restored from {@see SimpleHtmlDom::$noise}
	 *
	...
	{
		return @simplehtmldom_str_get_html("<$name>$value</$name>")->firstChild();
	}

	function createTextNode($value)
	{
		return @end(simplehtmldom_str_get_html($value)->nodes);
	}
