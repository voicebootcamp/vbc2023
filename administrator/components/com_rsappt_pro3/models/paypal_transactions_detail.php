...
class paypal_transactions_detailModelpaypal_transactions_detail extends JModelLegacy
{
...
/*		if (!$row->check()) {
		...
		/**
	 * Method to (un)publish a paypal_transactions
	 *
	 * @access	public
	 * @return	boolean	True on success
	 * @since	1.5
	...
	function publish($cid = array(), $publish = 1)
	{
		...
		if (sv_count_($cid ))
		{
			...
			$query = 'UPDATE '.$this->_table_prefix.'paypal_transactions'
				. ' SET published = ' . intval( $publish )
				. ' WHERE id_paypal_transactions IN ( '.$cids.' )'
	...
	/**
	 * Method to move a paypal_transactions_detail
	 *
	 * @access	public
	 * @return	boolean	True on success
	 * @since	1.5
	...
	{
		//DEVNOTE: Load table class from com_sv_ser/tables/paypal_transactions_detail.php		
		$row = $this->getTable();
		$groupings = array();
		...
		/**
	 * Method to move a paypal_transactions 
	 *
	 * @access	public
	 * @return	boolean	True on success
	 * @since	1.5
	...
	//DEVNOTE: Load table class from com_sv_ser/tables/paypal_transactions_detail.php	
		$row = $this->getTable();
	//DEVNOTE: we need to pass here id of paypal_transactions detail 		
		if (!$row->load($this->_id_paypal_transactions)) {
			$this->setError($row->getError());
