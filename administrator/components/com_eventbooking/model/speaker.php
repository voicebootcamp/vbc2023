...
class EventbookingModelSpeaker extends RADModelAdmin
{
	/**
	...
	protected function afterStore($row, $input, $isNew)
	{
		...
		if (!$isNew)
		{
			$query->delete('#__eb_event_speakers')
				->where('speaker_id = ' . $row->id);
			$db->setQuery($query)
				->execute();
		}
		...
		{
			$query->clear()
				->insert('#__eb_event_speakers')->columns('speaker_id, event_id');
