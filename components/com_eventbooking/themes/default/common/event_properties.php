...
<table class="<?php echo $bootstrapHelper->getClassMapping('table table-bordered table-striped'); ?>">
	...
	{
		$width  = (int) $config->map_width ?: 500;
		$height = (int) $config->map_height ?: 400;
		?>
		<tr class="eb-event-property">
			<td class="eb-event-property-label">
			...
			<td class="eb-event-property-value">
				...
				if ($location->address)
				{
					...
					{
					?>
						<a href="<?php echo Route::_('index.php?option=com_eventbooking&view=map&location_id=' . $item->location_id . '&Itemid=' . $Itemid . '&tmpl=component&format=html'); ?>"
