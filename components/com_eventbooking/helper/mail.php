...
class EventbookingHelperMail
{
	...
	public static function sendEmails($row, $config)
	{
		...
		EventbookingHelper::setEventMessagesDataFromCategory($event, $category,
			['admin_email_body', 'user_email_body', 'user_email_body_offline', 'group_member_email_body'], $fieldSuffix);
		...
		$collectMemberInformation = EventbookingHelperRegistration::isCollectMembersInformation($event, $config);
			...
			&& ($config->send_invoice_to_admin || $config->send_invoice_to_customer))
		{
			...
			if (!$invoiceFilePath)
			{
				$invoiceFilePath = JPATH_ROOT . '/media/com_eventbooking/invoices/' . EventbookingHelper::callOverridableHelperMethod('Helper',
						'formatInvoiceNumber', [$row->invoice_number, $config, $row]) . '.pdf';
			}
		}
	...
	protected static function sendRegistrationEmailToGroupMembers($mailer, $row, $rowMembers, $replaces, $attachments)
	{
		...
		foreach ($rowMembers as $rowMember)
		{
			...
			$memberReplaces['member_detail'] = EventbookingHelperRegistration::getMemberDetails($config, $rowMember, $event, $rowLocation, true,
				$memberForm);
			...
			if ($row->ticket_code && $row->payment_status == 1)
			{
				...
				{
					$ticketFilePaths = EventbookingHelper::callOverridableHelperMethod('Ticket', 'generateRegistrationTicketsPDF',
						[$rowMember, $config]);
	...
	public static function sendRegistrationApprovedEmail($row, $config)
	{
		...
		if ($fieldSuffix && isset($event->{'registration_approved_email_body' . $fieldSuffix}) && EventbookingHelper::isValidMessage($event->{'registration_approved_email_body' . $fieldSuffix}))
		{
	...
	public static function sendWaitingListNotificationEmail($row, $config)
	{
		...
		if (count($registrants))
		{
			...
			{
				$replaces['event_link'] = Uri::getInstance()->toString([
						'scheme',
						'user',
						'pass',
						'host',
					]) . Route::_(EventbookingHelperRoute::getEventRoute($row->event_id, 0, EventbookingHelper::getItemid()));
			}
			else
			{
	...
	public static function sendReminder($numberEmailSendEachTime = 0, $bccEmail = null, $params = null)
	{
		...
		$query->select('a.*')
			->select('b.from_name, b.from_email, b.reminder_email_body, b.reminder_email_subject')
			->select($db->quoteName(['c.reminder_email_subject', 'c.reminder_email_body'], ['cat_reminder_email_subject', 'cat_reminder_email_body']))
		...
		for ($i = 0, $n = count($rows); $i < $n; $i++)
		{
			...
			if (strlen($row->reminder_email_subject))
			{
				$emailSubject = $row->reminder_email_subject;
			}
			elseif (strlen($row->cat_reminder_email_subject))
			{
				$emailSubject = $row->cat_reminder_email_subject;
			}
			elseif ($fieldSuffix && strlen($message->{'reminder_email_subject' . $fieldSuffix}))
			{
			...
			elseif (EventbookingHelper::isValidMessage($row->cat_reminder_email_body))
			{
				$emailBody = $row->cat_reminder_email_body;
			}
	...
	public static function sendSecondReminder($numberEmailSendEachTime = 0, $bccEmail = null, $params = null)
	{
		...
		$query->select('a.*')
			->select('b.from_name, b.from_email, b.second_reminder_email_body, b.second_reminder_email_subject')
			->select($db->quoteName(['c.second_reminder_email_subject', 'c.second_reminder_email_body'],
				['cat_second_reminder_email_subject', 'cat_second_reminder_email_body']))
			->from('#__eb_registrants AS a')
			->innerJoin('#__eb_events AS b ON a.event_id = b.id')
			->leftJoin('#__eb_categories AS c ON b.main_category_id = c.id')
		...
		for ($i = 0, $n = count($rows); $i < $n; $i++)
		{
			...
			if (strlen($row->second_reminder_email_subject))
			{
				$emailSubject = $row->second_reminder_email_subject;
			}
			elseif (strlen($row->cat_second_reminder_email_subject))
			{
				$emailSubject = $row->cat_second_reminder_email_subject;
			}
			elseif ($fieldSuffix && strlen($message->{'second_reminder_email_subject' . $fieldSuffix}))
			{
			...
			elseif (EventbookingHelper::isValidMessage($row->cat_second_reminder_email_body))
			{
				$emailBody = $row->cat_second_reminder_email_body;
			}
			...
			foreach ($replaces as $key => $value)
			{
				$key          = strtoupper($key);
				$value        = (string) $value;
				$emailSubject = str_ireplace("[$key]", $value, $emailSubject);
				$emailBody    = str_ireplace("[$key]", $value, $emailBody);
			}
	...
	public static function sendThirdReminder($numberEmailSendEachTime = 0, $bccEmail = null, $params = null)
	{
		...
		$hourFrequencyConditionReminderBefore = "b.send_third_reminder >= TIMESTAMPDIFF(HOUR, $now, b.event_date) AND TIMESTAMPDIFF(HOUR, $now, b.event_date) >= 0";
		$dayFrequencyConditionReminderBefore  = "b.send_third_reminder >= DATEDIFF(b.event_date, $now) AND DATEDIFF(b.event_date, $now) >= 0";
		$dayFrequencyConditionReminderAfter   = "DATEDIFF($now, b.event_date) >= ABS(b.send_third_reminder) AND DATEDIFF($now, b.event_date) <= 60";
		$hourFrequencyConditionReminderAfter  = "TIMESTAMPDIFF(HOUR, b.event_date, $now) >= ABS(b.send_third_reminder) AND TIMESTAMPDIFF(HOUR, b.event_date, $now) <= 100";

		$query->select('a.*')
			->select('b.from_name, b.from_email, b.third_reminder_email_body, b.third_reminder_email_subject')
			->select($db->quoteName(['c.third_reminder_email_subject', 'c.third_reminder_email_body'],
				['cat_third_reminder_email_subject', 'cat_third_reminder_email_body']))
			->from('#__eb_registrants AS a')
			->innerJoin('#__eb_events AS b ON a.event_id = b.id')
			->leftJoin('#__eb_categories AS c ON b.main_category_id = c.id')
			->where('a.is_third_reminder_sent = 0')
			->where('b.send_third_reminder != 0')
			->where("IF(b.send_third_reminder > 0, IF(b.third_reminder_frequency = 'd', $dayFrequencyConditionReminderBefore, $hourFrequencyConditionReminderBefore), IF(b.third_reminder_frequency = 'd', $dayFrequencyConditionReminderAfter, $hourFrequencyConditionReminderAfter))")
			->order('b.event_date, a.register_date');
		...
		for ($i = 0, $n = count($rows); $i < $n; $i++)
		{
			...
			if (strlen($row->third_reminder_email_subject))
			{
				$emailSubject = $row->third_reminder_email_subject;
			}
			elseif (strlen($row->cat_third_reminder_email_subject))
			{
				$emailSubject = $row->cat_third_reminder_email_subject;
			}
			elseif ($fieldSuffix && strlen($message->{'third_reminder_email_subject' . $fieldSuffix}))
			{
				$emailSubject = $message->{'third_reminder_email_subject' . $fieldSuffix};
			}
			else
			{
				$emailSubject = $message->third_reminder_email_subject;
			}

			if (EventbookingHelper::isValidMessage($row->third_reminder_email_body))
			{
				$emailBody = $row->third_reminder_email_body;
			}
			elseif (EventbookingHelper::isValidMessage($row->cat_third_reminder_email_body))
			{
				$emailBody = $row->cat_third_reminder_email_body;
			}
			elseif ($fieldSuffix && EventbookingHelper::isValidMessage($message->{'third_reminder_email_body' . $fieldSuffix}))
			{
				$emailBody = $message->{'third_reminder_email_body' . $fieldSuffix};
			}
			else
			{
				$emailBody = $message->third_reminder_email_body;
			}
	...
	protected static function addRegistrationIcs($mailer, $row, $event, $config)
	{
		...
		if ($config->multiple_booking)
		{
			$db    = Factory::getDbo();
			$query = $db->getQuery(true)
				->select('a.id, a.title, a.event_date, a.event_end_date, a.short_description, l.lat, l.long, l.address AS location_address')
	...
	public static function sendRegistrantsListEmail($event, $attachment, $params)
	{
		...
		static::send($mailer, $emails, $subject, $body, $logEmails, 2, 'registrants_list_email');
	}
