...
class EventbookingControllerRegister extends EventbookingController
{
	...
	public function individual_registration()
	{
		...
		if (!$eventId)
		{
			Factory::getApplication()->enqueueMessage(Text::_('Invalid Event ID: 0'), 'warning');
		...
		if (!$event)
		{
			Factory::getApplication()->enqueueMessage(Text::_('EB_EVENT_NOT_FOUND'), 'warning');
	...
	protected function validateTickets($event, $input, &$data, $waitingListEnabled)
	{
		...
		if ($params->get('only_allow_register_one_ticket_type') && $numberSelectedTicketTypes > 1)
		{
			$errors[] = Text::_('EB_YOU_CAN_ONLY_REGISTER_FOR_ONE_TICKET_TYPE');
		}
