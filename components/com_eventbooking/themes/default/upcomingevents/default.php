...
<div id="eb-upcoming-events-page-default" class="eb-container">
	...
	if (count($this->items))
	{
		if (EventbookingHelperHtml::isLayoutOverridden('common/events_default.php'))
		{
			$layoutData = [
				'events'          => $this->items,
				'config'          => $this->config,
		...
		{
			echo $this->loadCommonLayout('common/events_default_layout.php');
		}
	}
	else
	{
	...
	?>
	<form method="post" name="adminForm" id="adminForm" action="<?php echo Route::_('index.php?option=com_eventbooking&view=upcomingevents&layout=default&Itemid=' . $this->Itemid); ?>">
