...
$results              = $dispatcher->triggerEvent('onContentBeforeDisplay',
    array($this->category->extension.'.categories', &$this->category, &$this->params, 0));
...
<div class="blog<?php echo $this->pageclass_sfx; ?>" itemscope itemtype="https://schema.org/Blog">
    ...
    <?php if ($beforeDisplayContent || $afterDisplayContent || $this->params->get('show_description', 1) || $this->params->def('show_description_image',
            1)) : ?>
    ...
    <?php if ( ! empty($this->lead_items)) : ?>
      <div class="qx-child-width-1-1 qx-grid-row-large" qx-grid>
          <?php foreach ($this->lead_items as &$item) : ?>
            <article class="qx-article leading-<?php echo $leadingcount; ?><?php echo $item->state == 0 ? ' system-unpublished' : null; ?>"
    ...
    <?php if ( ! empty($this->intro_items)) : ?>
        <?php foreach ($this->intro_items as $key => &$item) : ?>
          ...
          <div class="qx-child-width-1-<?php echo (int) $this->columns; ?>@m qx-child-width-1-1 qx-grid-row-large" qx-grid>
            <?php endif; ?>
        <article class="qx-article <?php echo $item->state == 0 ? ' system-unpublished' : null; ?> qx-margin-remove-top"
                 itemprop="blogPost" itemscope itemtype="https://schema.org/BlogPosting">
            <?php
            $this->item = &$item;
            echo $this->loadTemplate('item');
