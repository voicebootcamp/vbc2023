 ...
 * @since 2.0
 */
class WizardController extends JMapController {
	/**
	...
	public function createEntity() {
		...
		if (!$targetExtension) {
			...
			$this->setRedirect("index.php?option=$option&task=sources.display", Text::_('COM_JMAP_ERROR_NOTARGET_EXTENSION'));
			return false;
		}
		...
		if (!$model->createEntityProcess()) {
			...
			switch ($client) {
			case 'jsapp':
				$this->app->enqueueMessage (Text::_('COM_JMAP_ERROR_CREATING_DATASOURCE'), 'message');
			...
			// Switch client user controller make redirect to wizard.display on exceptions detected
				$this->setRedirect("index.php?option=$option&task=sources.display", Text::_('COM_JMAP_ERROR_CREATING_DATASOURCE'));
				return false;
			}
		}
		...
		switch ($client) {
		case 'jsapp':
			$this->app->enqueueMessage(sprintf(Text::_('COM_JMAP_SUCCESS_CREATING_DATASOURCE'), ucfirst(str_replace('_', ' ', $targetExtension))), 'message');
			$this->persistMessageQueue();
			return false;
			break;
		...
		default;
			// Switch client user controller make redirect to wizard.display on exceptions detected
			$this->setRedirect("index.php?option=$option&task=sources.display", sprintf(Text::_('COM_JMAP_SUCCESS_CREATING_DATASOURCE'), ucfirst(str_replace('_', ' ', $targetExtension))));
			return true;
		}
	}
}